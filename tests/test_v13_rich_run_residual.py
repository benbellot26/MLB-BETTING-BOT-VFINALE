...
class RichRunResidualTests(unittest.TestCase):
    def modules(self):
        return {name:{"status":"ACTIVE","coverage":1.0,"home_factor":1.03,"away_factor":.98} for name in rich.MODULES}

    def test_feature_vector_excludes_market(self):
        selected=("starter_ip","statcast")
        x=rich._vector(4.8,"home",self.modules(),selected)
        self.assertEqual(len(x),2)
        self.assertTrue(all(isinstance(v,float) for v in x))

    def test_adjustment_is_bounded(self):
        model={"selected_modules":["starter_ip","statcast"],"coefficients":[50.0,50.0]}
    ...
    def test_weather_is_not_a_historical_candidate(self):
        self.assertNotIn("weather_park",rich.MODULES)
        self.assertTrue(all("weather_park" not in s for s in rich.CANDIDATE_SETS))
    ...
    def test_shadow_never_changes_production_fields(self):
        ...
        artifact={"schema":rich.SCHEMA,"shadow_enabled":True,"status":"HISTORICALLY_VALIDATED_SHADOW",
                  "model":{"selected_modules":["statcast"],"coefficients":[.1]}}
