...
class V144ProfessionalDataModelTests(unittest.TestCase):
    def test_phase_calibration_keeps_one_snapshot_per_game_phase(self):
        ...
        self.assertEqual(artifact["calibrators"]["MARKET:ML"]["n"],1); self.assertEqual(artifact["calibrators"]["PHASE:EARLY:ML"]["n"],1); self.assertEqual(artifact["calibrators"]["PHASE:FINAL:ML"]["n"],1)

    def test_total_push_is_excluded_from_binary_calibration(self):
        row=settled(9,"2026-08-25T23:00:00Z","2026-08-25T22:00:00Z","FINAL",home_score=5,away_score=3,total_line=8.0)
        obs=observations([row])
        self.assertEqual(len(obs.get("ML") or []),1)
        self.assertEqual(len(obs.get("TOTAL_OVER") or []),0)
    ...
    def test_sharp_benchmark_is_strictly_paired(self):
        ...
        self.assertEqual(report["markets"]["ML"]["n"],2); self.assertEqual(sharp["paired_n"],1); self.assertEqual(sharp["model_paired"]["n"],1)
    ...
    def test_certification_can_authorize_only_one_market(self):
        ...
        out=certify(perf,cal,paper)
        self.assertTrue(out["certified"]); self.assertTrue(out["markets"]["ML"]["betting_certified"]); self.assertFalse(out["markets"]["TOTAL_OVER"]["betting_certified"])
    ...
    def test_statcast_staleness_is_visible_and_not_promotion_ready(self):
        ...
        out=build_shadow_features({"context":{},"features":{}},target_date="2026-08-25",artifact=artifact); self.assertEqual(out["freshness"],"STALE"); self.assertFalse(out["promotion_ready"])

    def test_integer_total_has_explicit_push_mass(self):
        p=RunProjection(game_pk="1",game_date="2026-08-25T20:00:00Z",analyzed_at="2026-08-25T18:00:00Z",home="H",away="A",home_mu=4.5,away_mu=4.0,total_line=8.5); out=total_probabilities(p,line=8.0)
        self.assertEqual(out["line_type"],"INTEGER"); self.assertGreater(out["push_probability"],0); self.assertAlmostEqual(out["complement_check"],1.0,places=10)
    ...
    def test_execution_market_line_shops_verified_books(self):
        ...
        out=best_execution(event,total_line=8.5,as_of="2026-08-25T18:00:00Z"); self.assertEqual(out["selections"]["home_ml"]["price"],1.95); self.assertEqual(out["selections"]["home_ml"]["bookmaker"],"pinnacle")
    ...
    def test_pa_simulator_is_calibrated_to_structural_full_nine_mean(self):
        ...
        for probability,bases,runs in _advance_transitions("1B",(True,False,False)):
            self.assertGreater(probability,0); self.assertEqual(sum(bool(v) for v in bases)+runs,2)
