...
def settled(game_pk, game_date, analyzed_at, phase, home_score=5, away_score=3, sharp=True):
    row={"model_generation":MODEL_GENERATION,"game_pk":str(game_pk),"game_date":game_date,"target_date":game_date[:10],"analyzed_at":analyzed_at,"phase":phase,"settled":True,"home_score":home_score,"away_score":away_score,"home_mu":4.5,"away_mu":4.0,"total_line":8.5,"probabilities":{"home_ml":.60,"away_ml":.40,"home_minus_1_5":.42,"away_plus_1_5":.58,"away_minus_1_5":.24,"home_plus_1_5":.76,"over":.55,"under":.45},"raw_probabilities":{"home_ml":.61,"away_ml":.39,"home_minus_1_5":.43,"away_plus_1_5":.57,"away_minus_1_5":.23,"home_plus_1_5":.77,"over":.56,"under":.44}}
...
class V144ProfessionalDataModelTests(unittest.TestCase):
    def test_phase_calibration_keeps_one_snapshot_per_game_phase(self):
        ...
        self.assertEqual(artifact["calibrators"]["MARKET:ML"]["n"],1)
        self.assertEqual(artifact["calibrators"]["PHASE:EARLY:ML"]["n"],1)
        self.assertEqual(artifact["calibrators"]["PHASE:FINAL:ML"]["n"],1)

    def test_sharp_benchmark_is_strictly_paired(self):
        rows=[settled(1,"2026-08-25T20:00:00Z","2026-08-25T18:00:00Z","FINAL",sharp=True),settled(2,"2026-08-25T21:00:00Z","2026-08-25T18:30:00Z","FINAL",sharp=False)]
        report=performance_report(rows); sharp=report["markets"]["ML"]["sharp_benchmark"]
        self.assertEqual(report["markets"]["ML"]["n"],2)
        self.assertEqual(sharp["paired_n"],1)
        self.assertEqual(sharp["model_paired"]["n"],1)
    ...
    def test_certification_can_authorize_only_one_market(self):
        ...
        for market in ("ML","RL_HOME_-1.5","RL_AWAY_-1.5","TOTAL_OVER"):
            good=market=="ML"
            markets[market]={"n":500 if good else 20,"ece":.02 if good else .20,"sharp_benchmark":{"paired_n":500 if good else 20,"brier_gain_ci95_lower":.002 if good else -.02,"logloss_gain_ci95_lower":0.0 if good else -.02,"brier_gain_vs_sharp":.01,"logloss_gain_vs_sharp":.01}}
            calibrators[f"MARKET:{market}"]={"accepted":good,"active":False,"status":"VALIDATED_IDENTITY" if good else "COLLECTING"}
        perf={"schema":"pulsar-v14-performance-v5","model_generation":MODEL_GENERATION,"games_settled":700,"markets":markets}
        paper={"by_market":{"ML":{"clv":{"n":120,"mean_clv":.7,"positive_rate":.56,"mean_clv_ci95_lower":.1}}}}
        out=certify(perf,{"calibrators":calibrators},paper)
        self.assertTrue(out["certified"])
        self.assertTrue(out["markets"]["ML"]["betting_certified"])
        self.assertFalse(out["markets"]["TOTAL_OVER"]["betting_certified"])
    ...
    def test_residual_home_offense_uses_away_pitching(self):
        ...
        x=offense_vs_opponent_features(row,"home")
        self.assertAlmostEqual(x[FEATURE_NAMES.index("opp_starter_era")],7.0)
        self.assertAlmostEqual(x[FEATURE_NAMES.index("off_lineup_ops")],.800)

    def test_statcast_staleness_is_visible_and_not_promotion_ready(self):
        artifact={"schema":"v13-7-statcast-id-priors-v1","point_in_time":True,"stable_id_only":True,"cutoff_day":"2026-08-01","hitters":{},"pitchers":{}}
        out=build_shadow_features({"context":{},"features":{}},target_date="2026-08-25",artifact=artifact)
    ...
    def test_integer_total_has_explicit_push_mass(self):
        p=RunProjection(game_pk="1",game_date="2026-08-25T20:00:00Z",analyzed_at="2026-08-25T18:00:00Z",home="H",away="A",home_mu=4.5,away_mu=4.0,total_line=8.5)
        out=total_probabilities(p,line=8.0)
        self.assertEqual(out["line_type"],"INTEGER")
        self.assertGreater(out["push_probability"],0)
        self.assertAlmostEqual(out["complement_check"],1.0,places=10)

    def test_weather_needs_venue_baseline_for_promotion(self):
        raw={"temperature_f":80,"humidity_pct":50,"pressure_hpa":1010,"wind_mph":10,"wind_direction_deg":180,"outfield_bearing_deg":0}
        diagnostic=environment(raw); self.assertFalse(diagnostic["promotion_ready"])
        ready=environment({**raw,"venue_baseline_density_kg_m3":1.18,"venue_baseline_wind_out_mph":2.0}); self.assertTrue(ready["promotion_ready"])

    def test_historical_backfill_rejects_live_season_endpoint(self):
        with self.assertRaisesRegex(ValueError,"historical leakage"):
            reject_live_season_backfill(target_game_date="2025-05-01T20:00:00Z",retrieved_at="2026-08-25T00:00:00Z",source_type="mlb_stats_season_live")

    def test_execution_market_line_shops_verified_books(self):
        event={"home_team":"Home","away_team":"Away","bookmakers":[{"key":"winamax_fr","last_update":"2026-08-25T17:59:00Z","markets":[{"key":"h2h","outcomes":[{"name":"Home","price":1.90},{"name":"Away","price":2.00}]}]},{"key":"pinnacle","last_update":"2026-08-25T17:59:00Z","markets":[{"key":"h2h","outcomes":[{"name":"Home","price":1.95},{"name":"Away","price":1.95}]}]}]}
