...
from v14.api_budget import KIND_CLOSE, used as api_used
...
class AuditHardeningTests(unittest.TestCase):
    ...
    def test_coverage_records_skips_without_network(self) -> None:
        ...
        self.assertEqual(len(rows),2);self.assertTrue(rows[0]["eligible"]);self.assertFalse(rows[1]["eligible"]);self.assertEqual(rows[1]["rejection_reason"],"odds_event_unmatched");self.assertTrue(all(r["network_calls_added"]==0 for r in rows))
    ...
    def test_cost_gate_does_not_call_loader_when_nothing_due(self) -> None:
        with TemporaryDirectory() as tmp:
            root=Path(tmp);market=root/"market.jsonl";paper=root/"paper.jsonl";bet=root/"bet.jsonl";usage=root/"usage.jsonl";called={"n":0}
            def loader():called["n"]+=1;return []
            out=cost_aware_run(market,paper,bet,api_usage_path=usage,events_loader=loader,now=datetime(2026,9,1,tzinfo=timezone.utc))
            self.assertFalse(out["api_call_performed"]);self.assertFalse(out["budget_reserved"]);self.assertEqual(out["paid_api_snapshots"],0);self.assertEqual(called["n"],0);self.assertEqual(api_used(KIND_CLOSE,usage,now=datetime(2026,9,1,tzinfo=timezone.utc)),0)

    def test_paid_gate_starts_only_inside_certified_close_window(self) -> None:
        self.assertEqual(CERTIFIED_DUE_WINDOW_MINUTES,15.0)
        with TemporaryDirectory() as tmp:
            root=Path(tmp);market=root/"market.jsonl";paper=root/"paper.jsonl";bet=root/"bet.jsonl";now=datetime(2026,9,1,12,0,tzinfo=timezone.utc)
            row={"schema":"pulsar-v14-market-close-v1","game_pk":"1","game_date":(now+timedelta(minutes=16)).isoformat(),"odds_event_time_verified":True,"close_history":[],"best_close":None}
            market.write_text(json.dumps(row)+"\n",encoding="utf-8");self.assertEqual(due_games(market,paper,bet,now=now),[])
            row["game_date"]=(now+timedelta(minutes=15)).isoformat();market.write_text(json.dumps(row)+"\n",encoding="utf-8");due=due_games(market,paper,bet,now=now);self.assertEqual(len(due),1);self.assertEqual(due[0]["source"],"MARKET")

    def test_paid_close_budget_is_reserved_before_loader_failure(self) -> None:
        with TemporaryDirectory() as tmp:
            root=Path(tmp);market=root/"market.jsonl";paper=root/"paper.jsonl";bet=root/"bet.jsonl";usage=root/"usage.jsonl";now=datetime(2026,9,1,12,0,tzinfo=timezone.utc)
            row={"schema":"pulsar-v14-market-close-v1","game_pk":"1","game_date":(now+timedelta(minutes=10)).isoformat(),"odds_event_time_verified":True,"close_history":[],"best_close":None}
            market.write_text(json.dumps(row)+"\n",encoding="utf-8")
            def failing_loader():raise RuntimeError("simulated paid request failure")
            with self.assertRaisesRegex(RuntimeError,"simulated paid request failure"):
                cost_aware_run(market,paper,bet,api_usage_path=usage,events_loader=failing_loader,now=now)
            self.assertEqual(api_used(KIND_CLOSE,usage,now=now),1)
