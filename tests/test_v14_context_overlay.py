...
from v14.context_overlay import MAX_TEAM_DELTA, bullpen_stress, context_overlay_from_feature_row, h2h_micro_signal, lineup_strength, starter_vulnerability
...
def bullpen(stressed):
    relievers = []
    for i in range(7):
        relievers.append({"name": f"R{i}", "taxed": stressed and i < 4, "likely_unavailable": stressed and i < 2, "uses_last_3d": 3 if stressed else 0, "pitches_last_3d": 55 if stressed else 8, "stats": {"era": 5.2 if stressed else 3.3, "whip": 1.48 if stressed else 1.15}})
    return {"coverage": 1.0, "relievers": relievers}


class ContextOverlayTests(unittest.TestCase):
    def test_weak_starter_scores_more_vulnerable(self):
    ...
    def test_real_lineup_required(self):
        self.assertTrue(lineup_strength(lineup(.800)).available)
        short = {"confirmed": True, "players": [{"ops": .800} for _ in range(6)]}
    ...
    def test_h2h_is_heavily_shrunk(self):
        tiny = h2h_micro_signal({"hits": 2, "at_bats": 2})
        larger = h2h_micro_signal({"hits": 15, "at_bats": 30})
        self.assertTrue(tiny.available)
        self.assertLess(abs(tiny.delta), 0.001)
        self.assertGreater(abs(larger.delta), abs(tiny.delta))
    ...
    def test_overlay_is_capped_and_market_free(self):
        row = {"point_in_time": True, "point_in_time_validation_reasons": [], "data_quality": {"eligible": True}, "context": {"home_starter": starter("Home", 3.0, 1.1, 2.0, 0.7, 10.0, 150), "away_starter": starter("Away", 6.2, 1.65, 4.5, 2.0, 5.5, 120), "home_lineup": lineup(.850), "away_lineup": lineup(.630)}, "features": {"home_bullpen": bullpen(False), "away_bullpen": bullpen(True)}, "rich_modules": {}}
