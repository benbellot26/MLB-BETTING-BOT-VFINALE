...
class HistoricalDistributionShadowTests(unittest.TestCase):
    def test_repository_artifact_is_hash_bound_and_shadow_only(self):
        artifact=load()
        self.assertEqual(artifact.get("status"),"HISTORICAL_VALIDATED_SHADOW")
        self.assertFalse(artifact.get("auto_activation"))
        self.assertFalse(artifact.get("champion_impact"))
        self.assertEqual((artifact.get("candidate_parameters") or {}).get("dispersion"),5.5)
