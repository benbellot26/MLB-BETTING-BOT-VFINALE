...
GAME_DATE="2026-08-25T23:00:00Z"; ANALYZED_AT="2026-08-25T22:30:00Z"; CLOSE_AT=datetime(2026,8,25,22,50,tzinfo=timezone.utc)
...
def _event(event_id="odds-123",include_total=True,commence_time=GAME_DATE):
    ...
    return {"id":event_id,"home_team":"Home","away_team":"Away","commence_time":commence_time,"bookmakers":[{"key":"pinnacle","last_update":"2026-08-25T22:49:00Z","markets":pinnacle_markets},{"key":"winamax_fr","last_update":"2026-08-25T20:00:00Z","markets":[{"key":"h2h","outcomes":[{"name":"Home","price":1.95},{"name":"Away","price":1.95}]}]}]}


def _paper_payload(analyzed_at=ANALYZED_AT,event_time=GAME_DATE,phase="FINAL",run_trigger=CERTIFICATION_RUN_TRIGGER):
    candidate={"selection":"home_ml","canonical_market":"ML","market":"ML","phase":phase,"price":2.00,"execution_book":"pinnacle","execution_source":"LINE_SHOPPED","probability":.60,"lower_probability":.56,"model_edge_pp":10.0,"robust_edge_pp":6.0,"sharp_edge_pp":6.0,"robust_sharp_edge_pp":2.0,"primary_sharp_benchmark":"PINNACLE_NO_VIG","pinnacle_probability":.54,"primary_sharp_edge_pp":6.0,"robust_primary_sharp_edge_pp":2.0,"primary_edge_qualified":True,"betting_edge_qualified":True,"sharp_source_count":1,"sharp_sportsbook_source_count":1,"sharp_exchange_proxy_source_count":0,"edge_qualified":True,"research_ready":True,"market_betting_certified":False,"status":"RESEARCH_ONLY"}
    sharp_row={"fair_probability":.54,"source_count":1,"sportsbook_source_count":1,"exchange_proxy_source_count":0,"contributors":[{"bookmaker":"pinnacle","source_type":"SPORTSBOOK","fair_probability":.54}]}
    return {"model_generation":MODEL_GENERATION,"run_trigger":run_trigger,"target_date":"2026-08-25","analyzed_at":analyzed_at,"results":[{"game_pk":"123","model_generation":MODEL_GENERATION,"run_trigger":run_trigger,"odds_event_id":"odds-123","game_date":GAME_DATE,"analyzed_at":analyzed_at,"phase":phase,"home":"Home","away":"Away","canonical_lines":{"TOTAL":8.5},"starter_fallback":{"degraded":False},"market_snapshot":{"freshness_verified":True,"commence_time":event_time,"markets":{"ML":{"bookmaker":"pinnacle"}}},"execution_market":{"freshness_verified":True,"selections":{"home_ml":{"price":2.00,"bookmaker":"pinnacle"}}},"sharp_market":{"freshness_verified":True,"selections":{"home_ml":sharp_row}},"v14_prediction":{"model_generation":MODEL_GENERATION,"run_trigger":run_trigger,"probability_policy_id":PROBABILITY_POLICY_ID,"phase":phase,"calibration":{"probability_policy_id":PROBABILITY_POLICY_ID},"probabilities":{"home_ml":.60},"raw_probabilities":{"home_ml":.61}},"decision":{"candidates":[candidate]}}]}
...
class V14LedgerTests(unittest.TestCase):
    ...
    def test_paper_entry_is_immutable_per_game_market(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger=Path(tmp)/"paper.jsonl"; self.assertEqual(record_paper(_paper_payload("2026-08-25T22:15:00Z"),ledger),1); self.assertEqual(record_paper(_paper_payload("2026-08-25T22:30:00Z"),ledger),0); out=report_paper(read_paper(ledger)); self.assertEqual(out["schema"],"pulsar-v14-paper-bet-performance-v8"); self.assertEqual(out["probability_policy_id"],PROBABILITY_POLICY_ID); self.assertEqual(out["certification_entry_phase"],"FINAL"); self.assertEqual(out["certification_run_trigger"],CERTIFICATION_RUN_TRIGGER); self.assertEqual(out["primary_clv_benchmark"],"PINNACLE_NO_VIG"); self.assertEqual(out["raw_observations"],1); self.assertEqual(out["observations"],1); self.assertEqual(out["by_market"]["ML"]["independent_games"],1); self.assertIn("SCHEDULED_FINAL",out["entry_policy"])
    ...
    def test_real_execution_ledger_requires_explicit_execution_facts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            execution={"authorized_bet_id":"123:ML","game_pk":"123","canonical_market":"ML","selection":"home_ml","executed_at":"2026-08-25T22:35:00Z","bookmaker":"pinnacle","odds":1.97,"stake_units":2.0,"stake_cash":20.0,"unit_value":10.0,"source":"MANUAL_TRACKER","result":"WIN","return_cash":39.4,"profit_cash":19.4,"profit_units":1.94}
