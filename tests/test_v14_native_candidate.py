...
def _builder(game, target_date, analyzed_at):
    ...
    return NativeGameInputs(
        ...
        home="Home",
        away="Away",
        context={"home": "Home", "away": "Away", "home_lineup": {"count": 0}, "away_lineup": {"count": 0}},
        feature_row=feature,
        structural_debug={"source": "test"},
    )
...
class V14NativeCandidateTests(unittest.TestCase):
    def test_candidate_is_native_non_publishing(self):
        ...
        self.assertEqual(candidate["role"], "CANDIDATE_NON_PUBLISHING")
        self.assertTrue(candidate["native_acquisition"])
        self.assertFalse(candidate["legacy_acquisition_adapter"])
    ...
    def test_parity_report_never_authorizes_cutover_by_itself(self):
        ...
        legacy = {"results": [{"game_pk": "123", "features": {"structural_home_mu": 4.4, "structural_away_mu": 4.3}}]}
        report = compare_with_legacy(candidate, legacy)
        self.assertEqual(report["comparable_games"], 1)
        self.assertAlmostEqual(report["max_abs_structural_run_delta"], .1, places=12)
