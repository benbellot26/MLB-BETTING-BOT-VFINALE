...
def _builder(game,target_date,analyzed_at):
    ...
    feature={"game_pk":"123","game_date":"2026-08-25T23:00:00Z","as_of":analyzed_at,"point_in_time":True,"point_in_time_validation_reasons":[],"data_quality":{"eligible":True,"starter_complete":starter_complete,"home_lineup_count":0,"away_lineup_count":0},"context":{"home_id":1,"away_id":2,"home_lineup":{"count":0},"away_lineup":{"count":0}},"features":{},"rich_modules":{}}
...
class V14NativeCandidateTests(unittest.TestCase):
    def test_candidate_is_native_non_publishing(self):
        ...
        self.assertEqual(research["weather_forecast_shadow"]["status"],"COLLECTING"); self.assertIn("network acquisition skipped",research["weather_forecast_shadow"]["reason"]); self.assertFalse(research["weather_forecast_shadow"]["champion_impact"])
        self.assertEqual(research["historical_team_run_shadow"]["status"],"READY_SHADOW"); self.assertEqual(research["historical_distribution_shadow"]["status"],"READY_SHADOW"); self.assertFalse(research["historical_team_run_shadow"]["champion_impact"]); self.assertFalse(research["historical_distribution_shadow"]["champion_impact"]); self.assertEqual(result["historical_team_run_shadow"],research["historical_team_run_shadow"]); self.assertEqual(result["historical_distribution_shadow"],research["historical_distribution_shadow"])
        self.assertEqual(result["training_features"]["schema"],"pulsar-v14-training-features-v8"); self.assertEqual(result["training_features"]["capture_mode"],"PROSPECTIVE_LIVE_SNAPSHOT"); self.assertTrue(result["training_features"]["pit_source_contract"]["live_pregame_safe"])
        ...
        self.assertIn("advanced_stats_adjustment",result["training_features"])
