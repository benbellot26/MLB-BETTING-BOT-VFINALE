...
class V14ProfessionalHardeningTests(unittest.TestCase):
    ...
    def test_unadjusted_exchange_proxies_are_discounted_below_pinnacle(self):
        self.assertLess(DEFAULT_SHARP_BOOK_WEIGHTS["betfair_ex_eu"],DEFAULT_SHARP_BOOK_WEIGHTS["pinnacle"])
        self.assertLess(DEFAULT_SHARP_BOOK_WEIGHTS["matchbook"],DEFAULT_SHARP_BOOK_WEIGHTS["pinnacle"])
    ...
    def test_unverified_sharp_market_is_not_actionable(self):
        event={"home_team":"Home","away_team":"Away","bookmakers":[{"key":"pinnacle","markets":[{"key":"h2h","outcomes":[{"name":"Home","price":1.9},{"name":"Away","price":2.0}]},{"key":"totals","outcomes":[{"name":"Over","point":8.5,"price":1.91},{"name":"Under","point":8.5,"price":1.91}]}]}]}; sharp=sharp_consensus(event,total_line=8.5,as_of="2026-08-25T18:00:00Z")
        self.assertFalse(sharp["actionable"]); self.assertFalse(sharp["freshness_verified"])

    def test_uncertainty_widens_when_starter_is_degraded(self):
        probabilities={"home_ml":.60,"away_ml":.40,"home_minus_1_5":.40,"away_plus_1_5":.60,"away_minus_1_5":.25,"home_plus_1_5":.75,"over":.55,"under":.45}; quality={"eligible":True,"home_lineup_count":9,"away_lineup_count":9}
        base=intervals(probabilities,{"markets":{}},data_quality=quality,starter_degraded=False,market_fresh=True); degraded=intervals(probabilities,{"markets":{}},data_quality=quality,starter_degraded=True,market_fresh=True)
    ...
    def test_decision_can_never_bet_when_not_certified(self):
        prediction={"probabilities":{"home_ml":.70,"away_ml":.30},"probability_intervals":{"selections":{"home_ml":{"lower":.64}}},"calibration":{"markets":{"ML":{"active":True}}}}
        market={"freshness_verified":True,"markets":{"ML":{"selections":{"home":{"price":1.80}}}}}; sharp={"freshness_verified":True,"selections":{"home_ml":{"fair_probability":.60,"source_count":2}}}
        ...
        self.assertEqual(out["best"]["status"],"RESEARCH_ONLY"); self.assertIn("betting_not_certified",out["best"]["blockers"])

    def test_decision_requires_lower_bound_to_beat_sharp(self):
        prediction={"probabilities":{"home_ml":.64,"away_ml":.36},"probability_intervals":{"selections":{"home_ml":{"lower":.54}}},"calibration":{"markets":{"ML":{"accepted":True}}}}
        market={"freshness_verified":True,"markets":{"ML":{"selections":{"home":{"price":2.05}}}}}; sharp={"freshness_verified":True,"selections":{"home_ml":{"fair_probability":.56,"source_count":2}}}; cert={"markets":{"ML":{"betting_certified":True}},"certified":True}
        out=decision_status(prediction=prediction,market_snapshot=market,sharp_market=sharp,certification=cert,starter_degraded=False)
        self.assertEqual(out["best"]["status"],"NO_BET"); self.assertLess(out["best"]["robust_sharp_edge_pp"],0)
    ...
    def test_probability_certification_does_not_skip_paper_clv_gate(self):
        ...
        performance={"schema":"pulsar-v14-performance-v5","model_generation":MODEL_GENERATION,"games_settled":700,"markets":markets}; calibration={"schema":"pulsar-v14-calibration-v2","model_generation":MODEL_GENERATION,"calibrators":calibrators}
        probability_only=certification_status(performance,calibration,{})
        self.assertTrue(probability_only["probability_certified"]); self.assertFalse(probability_only["certified"]); self.assertIn("paper_clv_n<100",probability_only["reasons"])
        paper={"schema":"pulsar-v14-paper-bet-performance-v3","by_market":{m:{"clv":{"n":120,"mean_clv":.35,"positive_rate":.56,"mean_clv_ci95_lower":.05}} for m in markets}}
        betting=certification_status(performance,calibration,paper); self.assertTrue(betting["probability_certified"]); self.assertTrue(betting["certified"]); self.assertEqual(betting["betting_status"],"BETTING_CERTIFIED")
