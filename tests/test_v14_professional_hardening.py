...
class V14ProfessionalHardeningTests(unittest.TestCase):
    ...
    def test_probability_certification_does_not_skip_executable_clv_gates(self):
        ...
        evidence={"n":120,"mean_clv":.35,"positive_rate":.56,"mean_clv_ci95_lower":.05,"mean_clv_ci95_upper":.60,"benchmark":"PINNACLE_NO_VIG","inference_method":"paired nonparametric bootstrap mean","bootstrap_reps":5000}; execution={"n":80,"mean_clv":.20,"positive_rate":.55,"mean_clv_ci95_lower":.02,"mean_clv_ci95_upper":.40,"inference_method":"paired nonparametric bootstrap mean","bootstrap_reps":5000}; paper={"schema":"pulsar-v14-paper-bet-performance-v8","model_generation":MODEL_GENERATION,"probability_policy_id":PROBABILITY_POLICY_ID,"generated_at":FRESH,"certification_entry_phase":"FINAL","certification_run_trigger":"SCHEDULED_FINAL","primary_clv_benchmark":"PINNACLE_NO_VIG","legacy_consensus_certification_clv_can_certify":False,"by_market":{m:{"latest_certified_close_at":FRESH,"latest_primary_close_at":FRESH,"latest_execution_close_at":FRESH,"certification_clv":dict(evidence),"execution_clv":dict(execution)} for m in markets}}
        betting=certification_status(performance,calibration,paper,now=NOW); self.assertTrue(betting["probability_certified"]); self.assertTrue(betting["certified"]); self.assertEqual(betting["betting_status"],"BETTING_CERTIFIED"); self.assertEqual((betting["policy"] or {}).get("paper_entry_phase"),"FINAL"); self.assertEqual((betting["policy"] or {}).get("paper_primary_clv_benchmark"),"PINNACLE_NO_VIG")
