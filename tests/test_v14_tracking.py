...
def payload(analyzed_at="2026-08-25T12:00:00Z"):
    return {
        ...
        "results":[{
            ...
            "v14_prediction":{"model_generation":MODEL_GENERATION,"run_projection":{"home_mu":4.8,"away_mu":3.9,"total_line":8.5},"probabilities":{"home_ml":.61,"away_ml":.39,"home_minus_1_5":.42,"away_plus_1_5":.58,"away_minus_1_5":.24,"home_plus_1_5":.76,"over":.54,"under":.46}},
        }],
    }
...
class V14TrackingTests(unittest.TestCase):
    def test_snapshot_is_idempotent_preserves_market_and_settles(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows=_read_jsonl(path); self.assertEqual(rows[0]["market_snapshot"]["markets"]["ML"]["selections"]["home"]["price"],1.90); self.assertEqual(rows[0]["market_diagnostics"]["markets"]["ML"]["selections"]["home"]["edge_pp"],3.1)
            def loader(day):
