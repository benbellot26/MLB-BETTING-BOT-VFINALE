...
for market,grid,base in [('ML',[.70,.75,.80,.85,.90,.95,1.0,1.05],1.0),('RL',[.45,.50,.55,.60,.65,.70,.75,.80,.82,.85,.90,.95],.82)]:
    ...
    for f in grid:
        ...
        bt=brier([obs(r,market) for r in parts['tune']],fun);lt=logloss([obs(r,market) for r in parts['tune']],fun)
        cand.append((bt,lt,f))
...
def candidate_rows(rs):
    ...
    for r in rs:
        for m in ('ML','RL'):
            ...
            if q<thr: continue
            dq=data_quality(r);ss=structural_stability(r);cal=market_rel(m);depth=.70
            out.append({'date':r['game_date'][:10],'game':r['game_pk'],'m':m,'q':q,'y':y,'strength':strength(q,m),'data':dq,'stability':ss,'cal':cal,'depth':depth})
    return out
...
STRATEGIES={
 ...
 'risk_balanced': {'strength':.25,'stability':.30,'data':.20,'cal':.15,'depth':.10},
}
...
def daily(rs,w):
    ...
    for day,z in d.items():
        z.sort(key=lambda c:(score(c,w),c['q']),reverse=True);used=set()
        for c in z:
            if c['game'] in used: continue
            cc=dict(c);cc['score']=score(c,w);picks.append(cc);used.add(c['game'])
            if len(used)>=3: break
