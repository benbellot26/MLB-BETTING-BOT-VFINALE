...
def _line(r):
    ...
    return (f"{state} • **{_label(r)}**\n"
            f"p V12.2 **{core.pct(r.get('p_effective'))}** • prudent **{core.pct(g.get('p_conservative'))}** • "
...
def send_game(result, portfolio):
    ...
    brief = (f"Phase **{result.get('phase')}** • modèle **{model.get('version') or 'structural-only'}**\n"
             ...
             f"Sharp ML refs **{int(core.num(con.get('n')))}** • dispersion **{core.num(model.get('dispersion')):.2f}** • env σ **{core.num(model.get('environment_sigma')):.3f}**")
    ...
    return core.send_embed(f"⚾ MLB V12.2 • {ctx['away']} @ {ctx['home']}", fields, 5763719)


def send_top(results):
    ok = True
    for m, title in (("ML", "🏆 TOP 3 ML V12.2"), ("RUNLINE", "⚾ TOP 3 RUN LINE V12.2"), ("TOTAL", "📈 TOP 3 TOTAL V12.2")):
        pool = []
        for r in results:
...
def send_plan(chosen, combo, portfolio, pool):
    simple = "\n\n".join(f"**#{i+1} {_label(c['rec'])} — {core.num((c['rec'].get('winamax_eval') or {}).get('official_units')):g}u recommandées**\nEV prudent {100*core.num(c['gate'].get('ev_at_price')):+.1f}% • DQ {100*core.num(c['dq'].get('score')):.0f}/100" for i, c in enumerate(chosen)) or "**AUCUNE RECOMMANDATION SIMPLE.**"
    ctext = "⚪ Combinés officiels désactivés en V12.2 tant que la dépendance n'est pas validée."
    status = f"Bankroll référence **{core.num(portfolio.get('bankroll_eur')):.2f}€** • exposition recommandée **{core.num(portfolio.get('allocated')):.2f}€** • staking **{portfolio.get('staking')}**"
    return core.send_embed("🎟️ RECOMMANDATIONS V12.2", [("✅ Simples", simple), ("🔗 Combiné", ctext), ("📊 Portfolio", status)], 5763719,
...
def send_health(h):
    ...
    txt = (f"Planifiés **{h.get('scheduled_games',0)}** • appariés **{h.get('matched_events',0)}** • analysés **{h.get('analyzed_games',0)}**\n"
           f"Starters confirmés/utilisables **{h.get('confirmed_starters',0)}/{h.get('usable_starters',0)}** • lineups complètes/utilisables **{h.get('confirmed_lineups',0)}/{h.get('usable_lineups',0)}** • DQ moyenne **{100*core.num(h.get('mean_data_quality')):.0f}/100**\n"
    ...
    return core.send_embed("🩺 DATA HEALTH V12.2", [("Couverture", txt)], 5763719)
