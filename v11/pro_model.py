...
def feature_dict(result_or_row):
    ...
    hs = starters.get("home") or {}
    aws = starters.get("away") or {}
    f.update({
...
def apply_run_correction(home_mu, away_mu, result_like, model=None):
    ...
    means = residual.get("means") or {}
    stds = residual.get("stds") or {}
    ...
    return max(1.4, home_mu + hd), max(1.4, away_mu + ad), {"active": True, "home_delta": hd, "away_delta": ad, "version": model.get("version")}
...
def calibrate(market, p, model=None):
    ...
    if model.get("active") and cal.get("active"):
        q = _sigmoid(_num(cal.get("a")) + _num(cal.get("b"), 1.0) * _logit(p))
        unc = max(config.MIN_MODEL_UNCERTAINTY, _num(cal.get("holdout_brier"), .25) ** .5 / math.sqrt(max(1, int(cal.get("train_n", 1)))))
        return max(.001, min(.999, q)), unc, "champion"
    return max(.001, min(.999, _num(p, .5))), config.FALLBACK_MODEL_UNCERTAINTY, "uncalibrated"
...
def build_candidate(rows):
    ...
    out = {
        "schema": "v11-champion-model-v1",
        "version": "v11-professional-challenger-1",
    ...
    if len(settled) >= 80:
        ...
        for r in settled:
            for mu_key, score_key in (("projected_home_runs", "home_score"), ("projected_away_runs", "away_score")):
                mu = max(.1, _num(r.get(mu_key)))
                y = _num(r.get(score_key))
                numer += mu*mu
                denom += max(0.0, (y-mu)**2-mu)
        if denom > 1e-9:
            out["run_dispersion"] = max(2.0, min(30.0, numer/denom))
    ...
    if len(settled) >= config.MIN_RESIDUAL_TRAIN_GAMES:
        ...
        yh = [_num(r.get("home_score")) - _num(r.get("projected_home_runs")) for r in train]
        ya = [_num(r.get("away_score")) - _num(r.get("projected_away_runs")) for r in train]
        hc = _ridge_fit(xs, yh)
        ac = _ridge_fit(xs, ya)
        ...
        for r, x in zip(hold, hold_x):
            bh = _num(r.get("projected_home_runs")); ba = _num(r.get("projected_away_runs"))
            ah = _num(r.get("home_score")); aa = _num(r.get("away_score"))
        ...
        out["residual"] = {
            "active": residual_pass,
            "n": len(settled), "train_n": len(train), "holdout_n": len(hold),
    ...
    for market in ("ML", "RUNLINE", "TOTAL"):
        ex = []
        for r in settled:
            opts = [o for o in r.get("options") or [] if o.get("market") == market and o.get("result") in {"WIN", "LOSS"}]
            if not opts:
                continue
            o = max(opts, key=lambda z: _num(z.get("p_model", z.get("p_effective")), .5))
            ex.append((_num(o.get("p_model", o.get("p_effective")), .5), 1 if o.get("result") == "WIN" else 0))
        ...
        cut = min(cut, len(ex)-max(20, len(ex)//10))
        ...
        passes = len(hold) >= config.MIN_CALIBRATION_HOLDOUT and gain >= config.MIN_CALIBRATION_BRIER_GAIN and mc["logloss"] <= mb["logloss"]
    ...
    out["passes"] = residual_pass or any(calibration_passes)
    return out
