...
import bot as core
from . import config
from .features import live_ml_features, snapshot_features, point_in_time_snapshot
from .models import load_model, apply_ml_heads, patch_ml_options, attach_market_challengers
from .selector import allocate
from .journal import load_rows, write_rows, settle_rows, capture_official_bets, comparison_summary, finance_summary, write_report
...
def main():
    ...
    comparison=comparison_summary(rows); finance=finance_summary(rows)
    report={"version":config.VERSION,"run_id":run_id,"analyzed_at":analyzed_at,"target_date":core.TARGET_DATE,"model":model,"remaining_games_analyzed":len(new),"settled_this_run":settled_now,"comparison":comparison,"official_finance":finance,"production":{"ML":"V11.3 direction + V11.2 probability","RUNLINE":"V10.0.15 until V11 challenger gate passes","TOTAL":"V10.0.15 until V11 challenger gate passes","selector":"V11 value-gated","price_gate":True},"challenger_policy":{"auto_promotion":False,"requires_walk_forward":True,"requires_live_confirmation":True,"gates":{"min_holdout_n":config.MIN_HOLDOUT_N,"min_live_n":config.MIN_LIVE_N,"min_brier_gain":config.MIN_BRIER_GAIN,"min_gain_probability":config.MIN_GAIN_PROB}},"top_picks":[{k:r.get(k) for k in ("game_pk","away","home","base_v10_pick","v11_3_pick","v11_3_direction_score","v11_2_probability_for_pick","grade","rank_score","phase")} for r in sorted(new,key=lambda x:x.get("rank_score",0),reverse=True)[:5]]}
