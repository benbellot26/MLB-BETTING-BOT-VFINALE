...
def required_price(rec):
    ...
    fair=1/p;ev_floor=(1+config.MIN_EV)/p;edge_floor=1/max(.001,p-config.MIN_EDGE)
...
def value_gate(rec):
    ...
    return {"ok":price>1 and price+1e-12>=minimum,"price":price if price>1 else None,"required_price":round(minimum,4),"ev_at_price":round(_num(rec.get("p_effective"),.5)*price-1,6) if price>1 else None}

def _score(result,rec):
    p=_num(rec.get("p_effective"),.5);conf=_num(rec.get("confidence"),0);q=_num(result.get("quality"),0);refs=int(_num(rec.get("refs"),0));phase=str(result.get("phase","EARLY")).upper()
    s=52+160*max(0,p-.5)+2.0*(conf-5)+13*(q-.6)+2*min(refs,4)+(5 if phase=="FINAL" else 2 if phase=="LATE" else 0)
...
def allocate(results,unit_eur=.5):
    ...
    pool.sort(key=lambda x:(x["score"],_num(x["rec"].get("p_effective"),.5),_num(x["rec"].get("confidence"),0)),reverse=True)
    ...
    for c in pool:
        ...
        units=2.0 if c["score"]>=86 and str(c["result"].get("phase")).upper()=="FINAL" else 1.0
        if used_units+units>config.MAX_DAILY_UNITS:units=1.0
        if used_units+units>config.MAX_DAILY_UNITS:continue
        e=c["rec"]["winamax_eval"];e.update({"official_selected":True,"official_units":units,"selected":True,"units":units,"stake_eur":round(units*unit_eur,2),"official_reason":f"V11 value gate: score {c['score']:.1f}/100, cote {c['gate']['price']:.2f} >= mini {c['gate']['required_price']:.2f}"})
        chosen.append(c);used_games.add(gid);profiles[profile]=profiles.get(profile,0)+1;used_units+=units
    legs=[];seen=set()
    for c in pool:
        if c["score"]<74:continue
    ...
    portfolio={"daily_cap":round(config.MAX_DAILY_UNITS*unit_eur,2),"allocated":round(total_units*unit_eur,2),"remaining":round(max(0,(config.MAX_DAILY_UNITS-total_units)*unit_eur),2),"official_count":len(chosen),"official_units":used_units,"combo_official":bool(combo.get("official")),"combo_units":_num(combo.get("units"),0),"selector_version":"V11-all-markets-value-v1"}
