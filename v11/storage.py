...
def snapshot_run(games, events, run_id, analyzed_at, target_date):
    """Compressed point-in-time run archive; workflow persists it as a durable release asset."""
    SNAPSHOT_DIR.mkdir(parents=True, exist_ok=True)
    path = SNAPSHOT_DIR/f"{target_date}_{run_id}.json.gz"
    payload = {
        "schema": "v12-raw-snapshot-v2", "run_id": run_id, "analyzed_at": analyzed_at,
    ...
    for event in events or []:
        ...
        for book in event.get("bookmakers") or []:
            for market in book.get("markets") or []:
                rows.append({
                    "schema": "v12-market-snapshot-v2", "run_id": run_id, "analyzed_at": analyzed_at,
...
def record_selected_bets(chosen, combo, run_id, analyzed_at, target_date):
    ...
    for c in chosen:
        ...
        if existing.get(key, {}).get("status") in {"PLACED", "PENDING", "WIN", "LOSS", "PUSH"}:
            continue
        rows.append({
            "schema": "v12-bet-ledger-v2", "event_type": "PLACED", "bet_key": key, "status": "PLACED",
            ...
            "bet_sharp_fair": rec.get("p_market"),
    ...
    if combo and combo.get("official"):
        sig = "||".join(sorted(
            bet_key(c["result"].get("game_pk"), c["rec"].get("market"), c["rec"].get("name"), c["rec"].get("point"))
        ...
        if key not in existing:
            rows.append({
                ...
                "combo_legs": [{
                    "game_pk": c["result"].get("game_pk"), "market": c["rec"].get("market"),
...
def update_clv(results, analyzed_at=None):
    ...
    for key, bet in open_bets.items():
        ...
        if not r:
            continue
        rec = next((
            o for o in r.get("options") or []
            if bet_key(r.get("game_pk"), o.get("market"), o.get("name"), o.get("point")) == key
        ), None)
        if not rec:
            continue
        ...
        row = {
            "schema": "v12-bet-ledger-v2", "event_type": "PRICE_OBSERVATION", "bet_key": key,
            "status": bet.get("status", "PLACED"), "observed_at": analyzed_at,
            "latest_price": price, "latest_sharp_fair": sharp,
        ...
        if minutes_to_start is not None and minutes_to_start > 0:
            if minutes_to_start <= config.T60_WINDOW_MIN and bet.get("t60_price") is None:
                row["t60_price"] = price
                row["t60_sharp_fair"] = sharp
                row["t60_observed_at"] = analyzed_at
            if minutes_to_start <= config.T15_WINDOW_MIN and bet.get("t15_price") is None:
                row["t15_price"] = price
                row["t15_sharp_fair"] = sharp
                row["t15_observed_at"] = analyzed_at
            if minutes_to_start <= config.CLOSING_CANDIDATE_WINDOW_MIN:
                row["close_candidate_price"] = price
                row["close_candidate_sharp_fair"] = sharp
                row["close_candidate_observed_at"] = analyzed_at
                row["close_candidate_minutes_to_start"] = round(minutes_to_start, 2)
                row["close_candidate_clv_pct"] = round(placed/price-1, 6) if placed > 1 else None
...
def settle_from_journal(rows, settled_at=None):
    ...
    for key, bet in state.items():
        ...
        if bet.get("bet_type") == "COMBO":
            ...
            pnl = units*(settled_price-1) if status == "WIN" and settled_price is not None else -units if status == "LOSS" else 0.0
            out.append({
                "schema": "v12-bet-ledger-v2", "event_type": "SETTLED", "bet_key": key, "status": status,
                "settled_at": settled_at, "profit_units": round(pnl, 4),
                "settled_price": settled_price, "leg_results": leg_status,
            })
            continue
...
def ledger_summary(events=None):
    ...
    settled = [v for v in state.values() if v.get("status") in {"WIN", "LOSS", "PUSH"}]
    stake = sum(_num(x.get("units")) for x in settled if x.get("status") != "PUSH")
    pnl = sum(_num(x.get("profit_units")) for x in settled)
    clv = [x.get("close_candidate_clv_pct") for x in state.values()
           if x.get("close_candidate_clv_pct") is not None and x.get("bet_type") != "COMBO"]
    sharp_move = [x.get("sharp_probability_move") for x in state.values()
                  if x.get("sharp_probability_move") is not None and x.get("bet_type") != "COMBO"]
    ordered = sorted(
        [x for x in settled if x.get("profit_units") is not None],
        key=lambda x: str(x.get("settled_at") or ""),
    )
    eq = peak = max_dd = 0.0
    losing = cur = 0
    for x in ordered:
        v = _num(x.get("profit_units"))
        eq += v
        peak = max(peak, eq)
    ...
    for market in ("ML", "RUNLINE", "TOTAL"):
        ...
        st = sum(_num(x.get("units")) for x in xs if x.get("status") != "PUSH")
        pu = sum(_num(x.get("profit_units")) for x in xs)
        if xs:
            by_market[market] = {
                "n": len(xs), "wins": sum(x.get("status") == "WIN" for x in xs),
                "losses": sum(x.get("status") == "LOSS" for x in xs),
                "pushes": sum(x.get("status") == "PUSH" for x in xs),
                "profit_units": round(pu, 4), "roi": pu/st if st else None,
            }

    return {
        "settled_singles": sum(x.get("bet_type") != "COMBO" for x in settled),
        "settled_combos": sum(x.get("bet_type") == "COMBO" for x in settled),
        "wins": sum(x.get("status") == "WIN" for x in settled),
        "losses": sum(x.get("status") == "LOSS" for x in settled),
        "pushes": sum(x.get("status") == "PUSH" for x in settled),
        "staked_units": round(stake, 4), "profit_units": round(pnl, 4),
        "roi": pnl/stake if stake else None, "by_market": by_market,
        ...
        "close_candidate_clv_n": len(clv),
        "mean_sharp_probability_move": sum(sharp_move)/len(sharp_move) if sharp_move else None,
        "sharp_move_n": len(sharp_move),
        "open_bets": sum(v.get("status") in {"PLACED", "PENDING"} for v in state.values()),
    }
