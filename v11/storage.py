...
def _read_jsonl(path):
    ...
    for line in p.read_text(encoding="utf-8").splitlines():
        ...
        except Exception: core.logging.warning("JSONL invalide ignoré: %s", p)
    return out
...
def capture_market_snapshot(events, run_id, analyzed_at, target_date):
        ...
        for book in event.get("bookmakers") or []:
            for m in book.get("markets") or []:
                rows.append({"schema": "v12-market-snapshot-v3", "run_id": run_id, "analyzed_at": analyzed_at,
                             "target_date": target_date, "event_id": event.get("id"), "home_team": event.get("home_team"),
    ...
    _append_jsonl(MARKET_SNAPSHOT_FILE, rows); return len(rows)
...
def load_ledger_events(path=None): return _read_jsonl(BET_LEDGER_FILE if path is None else path)
...
def fold_ledger(events=None):
    ...
    for e in (load_ledger_events() if events is None else events):
        key = e.get("bet_key")
        if key: state.setdefault(key, {}).update(e)
    return state


def open_recommendations(): return {k: v for k, v in fold_ledger().items() if v.get("status") in OPEN_RECOMMENDATION_STATUSES}
def open_exposure(): return {k: v for k, v in fold_ledger().items() if v.get("status") in OPEN_EXPOSURE_STATUSES}
...
def record_selected_bets(chosen, combo, run_id, analyzed_at, target_date):
    ...
    for c in chosen:
        ...
        if existing.get(key, {}).get("status") in OPEN_RECOMMENDATION_STATUSES | SETTLED_STATUSES: continue
    ...
    _append_jsonl(BET_LEDGER_FILE, rows); return len(rows)


def mark_run_published(run_id, published_at=None):
    published_at = published_at or datetime.now(timezone.utc).isoformat(); rows = []
...
def confirm_placed(key, actual_price=None, placed_at=None):
    ...
    if not state or state.get("status") not in {"PUBLISHED", "PROPOSED"}: raise KeyError(f"Recommandation introuvable ou non confirmable: {key}")
    ...
    if actual_price is not None: row["bet_price"] = _num(actual_price)
    _append_jsonl(BET_LEDGER_FILE, [row]); return row


def update_clv(results, analyzed_at=None):
    analyzed_at = analyzed_at or datetime.now(timezone.utc).isoformat(); now = _dt(analyzed_at) or datetime.now(timezone.utc)
    rows, by_game = [], {str(r.get("game_pk")): r for r in results}
    for key, bet in open_recommendations().items():
        if bet.get("bet_type") == "COMBO": continue
        ...
        minutes_to_start = (start-now).total_seconds()/60 if start else None; placed = _num(bet.get("bet_price"), 0); sharp = rec.get("p_market")
        row = {"schema": "v12-bet-ledger-v3", "event_type": "PRICE_OBSERVATION", "bet_key": key, "status": bet.get("status"),
               "observed_at": analyzed_at, "latest_price": price, "latest_sharp_fair": sharp,
        ...
        if bet.get("first_observed_price") is None:
            row["first_observed_price"] = price; row["first_observed_sharp_fair"] = sharp
...
def settle_from_journal(rows, settled_at=None):
    settled_at = settled_at or datetime.now(timezone.utc).isoformat(); finals = {}
    for r in rows:
    ...
    for key, bet in fold_ledger().items():
        if bet.get("status") != "CONFIRMED_PLACED": continue
        if bet.get("bet_type") == "COMBO":
            ...
            for leg in bet.get("combo_legs") or []:
                ...
                if not r: ready = False; break
                opt = {"market": leg.get("market"), "name": leg.get("pick"), "point": leg.get("point"), "p_effective": .5}; journal.settle_option(opt, r)
                st = opt.get("result")
                if st not in SETTLED_STATUSES: ready = False; break
                leg_status.append(st)
                if st == "WIN": win_prices.append(_num(leg.get("price"), 0))
            if not ready: continue
            if "LOSS" in leg_status: status, settled_price = "LOSS", None
            elif not win_prices: status, settled_price = "PUSH", 1.0
            elif all(x > 1 for x in win_prices): status, settled_price = "WIN", math.prod(win_prices)
            else: continue
            units = _num(bet.get("units")); pnl = units*(settled_price-1) if status == "WIN" else -units if status == "LOSS" else 0.0
            out.append({"schema": "v12-bet-ledger-v3", "event_type": "SETTLED", "bet_key": key, "status": status,
                        "settled_at": settled_at, "profit_units": round(pnl, 4), "settled_price": settled_price, "leg_results": leg_status}); continue
        r = (finals.get(str(bet.get("game_pk"))) or (None, None))[1]
        if not r: continue
        opt = {"market": bet.get("market"), "name": bet.get("pick"), "point": bet.get("point"), "p_effective": bet.get("p_effective")}; journal.settle_option(opt, r)
        status = opt.get("result")
        if status not in SETTLED_STATUSES: continue
        units, price = _num(bet.get("units")), _num(bet.get("bet_price")); pnl = units*(price-1) if status == "WIN" and price > 1 else -units if status == "LOSS" else 0.0
        out.append({"schema": "v12-bet-ledger-v3", "event_type": "SETTLED", "bet_key": key, "status": status,
                    "settled_at": settled_at, "profit_units": round(pnl, 4)})
    _append_jsonl(BET_LEDGER_FILE, out); return len(out)


def ledger_summary(events=None):
    state = fold_ledger(events); settled = [v for v in state.values() if v.get("status") in SETTLED_STATUSES]
    ...
    ordered = sorted([x for x in settled if x.get("profit_units") is not None], key=lambda x: str(x.get("settled_at") or "")); eq = peak = max_dd = 0.0; losing = cur = 0
    for x in ordered:
        v = _num(x.get("profit_units")); eq += v; peak = max(peak, eq); max_dd = max(max_dd, peak-eq)
        if v < 0: cur += 1; losing = max(losing, cur)
        elif v > 0: cur = 0
    return {"settled_singles": sum(x.get("bet_type") != "COMBO" for x in settled), "settled_combos": sum(x.get("bet_type") == "COMBO" for x in settled),
            "wins": sum(x.get("status") == "WIN" for x in settled), "losses": sum(x.get("status") == "LOSS" for x in settled),
            "pushes": sum(x.get("status") == "PUSH" for x in settled), "staked_units": round(stake, 4), "profit_units": round(pnl, 4),
            "roi": pnl/stake if stake else None, "max_drawdown_units": round(max_dd, 4), "longest_losing_streak": losing,
            "mean_close_candidate_clv_pct": sum(clv)/len(clv) if clv else None,
            "positive_close_candidate_clv_rate": sum(x > 0 for x in clv)/len(clv) if clv else None, "close_candidate_clv_n": len(clv),
            "mean_sharp_probability_move": sum(sharp_move)/len(sharp_move) if sharp_move else None, "sharp_move_n": len(sharp_move),
            "proposed": sum(v.get("status") == "PROPOSED" for v in state.values()),
