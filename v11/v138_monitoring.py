...
def provider_snapshot() -> dict[str, Any]:
    ...
    metrics = free.get("provider_metrics") or {}

    return {
        "observed_at": datetime.now(timezone.utc).isoformat(),
        "free_health_status": free.get("status"),
        "free_health_alerts": free.get("alerts") or [],
        "statcast_rows": stat.get("rows") or stat.get("accepted_pitch_rows"),
        "statcast_failures": stat.get("failed_chunks") or stat.get("failures"),
        "weather_available": weather.get("available_rows"),
        "weather_rows": weather.get("rows"),
        "weather_coverage": metrics.get("weather_coverage"),
        "weather_pit_coverage": metrics.get("weather_pit_coverage"),
        "weather_fallback_rows": weather.get("fallback_rows"),
        "park_rows": park.get("total_venue_rows"),
        "park_failed_requests": park.get("failed_requests"),
        "park_empty_parses": park.get("empty_parse_count"),
        "park_window_rejections": park.get("window_rejection_count"),
        "park_rolling_parameter": park.get("rolling_parameter"),
...
def build() -> dict[str, Any]:
    ...
    hist = _history()
    rows, _labels = models.load_free_dataset()
    ...
    if snap.get("free_health_status") == "DEGRADED":
    ...
    if snap.get("park_failed_requests"):
        alerts.append("park_factor_provider_failure")
    if snap.get("park_empty_parses"):
        alerts.append("park_factor_empty_parse")
    if snap.get("park_window_rejections"):
        alerts.append("park_factor_window_rejection")
    if snap.get("park_rolling_parameter") not in (None, 3):
        alerts.append("park_factor_wrong_rolling_window")
    for item in drift.get("alerts") or []:
        alerts.append(f"feature_drift:{item}")

    return {
        "schema": "v13-8-monitoring-v2",
        ...
        "claim": "observability only; no automatic model retuning",
    }
...
def render(report: dict[str, Any]) -> str:
    ...
    drift_rows = "".join(
        f"<tr><td>{esc(key)}</td><td>{esc(round(float(value.get('standardized_shift') or 0), 3))}</td>"
        f"<td>{'⚠️' if value.get('alert') else 'OK'}</td></tr>"
...
def main() -> None:
    ...
    print(json.dumps({"schema": report["schema"], "alerts": report["alerts"], "dashboard": str(DASHBOARD)}, indent=2))
