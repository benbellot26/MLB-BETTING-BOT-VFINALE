...
def bootstrap_difference(
    ...
    a: list[float],
    b: list[float],
    metric: Callable[[list[Any], list[float]], float | None],
    iterations: int = 600,
    seed: int = 138,
) -> dict[str, Any]:
    ...
    for _ in range(max(100, int(iterations))):
        ...
        if ma is not None and mb is not None:
            values.append(float(ma) - float(mb))
    values.sort()
    if not values:
        return {"available": False, "n": n}

    def q(frac: float) -> float:
        return values[min(len(values) - 1, max(0, int(frac * (len(values) - 1))))]
    ...
    return {
        ...
        "ci95_low": q(.025),
        "ci95_high": q(.975),
        "probability_a_better": sum(v < 0 for v in values) / len(values),
    }
...
def _paired(rows: list[dict[str, Any]], labels: list[dict[str, Any]]) -> list[tuple[dict[str, Any], dict[str, Any]]]:
    by = {
        str(l.get("game_pk")): l
        for l in labels
        if l.get("home_score") is not None and l.get("away_score") is not None
    }
    return [(r, by[str(r.get("game_pk"))]) for r in rows if str(r.get("game_pk")) in by]


def evaluate_predictions(
    paired: list[tuple[dict[str, Any], dict[str, Any]]],
    run_preds: list[tuple[float, float]],
) -> dict[str, Any]:
    yh = [_num(l.get("home_score")) for _, l in paired]
    ya = [_num(l.get("away_score")) for _, l in paired]
    ph = [x[0] for x in run_preds]
    pa = [x[1] for x in run_preds]
    ywin = [int(_num(l.get("home_score")) > _num(l.get("away_score"))) for _, l in paired]
    pwin = [home_win_probability(h, a) for h, a in run_preds]
    return {
        "n": len(paired),
        "home_rmse": models.rmse(yh, ph),
        "away_rmse": models.rmse(ya, pa),
        "home_mae": models.mae(yh, ph),
        "away_mae": models.mae(ya, pa),
        "ml_brier": brier(ywin, pwin),
        "ml_logloss": logloss(ywin, pwin),
        "ml_ece": expected_calibration_error(ywin, pwin),
        "calibration_bins": calibration_bins(ywin, pwin),
    }
...
def baseline_predictions(paired: list[tuple[dict[str, Any], dict[str, Any]]]) -> dict[str, Any]:
    ...
    for r, l in paired:
        ...
        if p is not None:
            market.append(_clip(_num(p, .5)))
            market_y.append(int(_num(l.get("home_score")) > _num(l.get("away_score"))))
    return {
        ...
        "market_if_present": (
            {"n": len(market_y), "brier": brier(market_y, market), "logloss": logloss(market_y, market)}
            if market_y else {"n": 0, "status": "NO_PIT_MARKET_IN_FREE_DATA"}
        ),
    }
...
def _season(row: dict[str, Any]) -> int | None:
    raw = row.get("season")
    if raw is None:
        raw = str(row.get("official_date") or row.get("game_date") or "")[:4]
    try:
        return int(raw)
    except Exception:
        return None
...
def walk_forward(rows: list[dict[str, Any]], labels: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    seasons = sorted({s for r, _ in paired if (s := _season(r)) is not None})
    folds = []
    for season in seasons[1:]:
        train = [(r, l) for r, l in paired if (_season(r) or season) < season]
        test = [(r, l) for r, l in paired if _season(r) == season]
        ...
        ywin = [int(_num(l.get("home_score")) > _num(l.get("away_score"))) for _, l in test]
...
def ablation_report(rows: list[dict[str, Any]], labels: list[dict[str, Any]]) -> dict[str, Any]:
    paired = _paired(rows, labels)
    if len(paired) < 300:
        return {"available": False, "n": len(paired), "minimum_n": 300}
    paired.sort(key=lambda z: str(z[0].get("game_date") or z[0].get("as_of") or ""))
    cut = int(len(paired) * .8)
    train, test = paired[:cut], paired[cut:]
    X = [models.vectorize(r) for r, _ in train]
    Xt = [models.vectorize(r) for r, _ in test]
    yh = [_num(l.get("home_score")) for _, l in train]
    ya = [_num(l.get("away_score")) for _, l in train]
    yht = [_num(l.get("home_score")) for _, l in test]
    yat = [_num(l.get("away_score")) for _, l in test]
    groups = {
        ...
        "rest": [13, 14],
        "park": [15],
        "talent_shrinkage": [16, 17],
        "offense_defense_interaction": [18, 19],
    }

    def score(drop: set[int]) -> float:
        keep = [j for j in range(len(models.FEATURE_NAMES)) if j not in drop]
        Xk = [[r[j] for j in keep] for r in X]
        Xtk = [[r[j] for j in keep] for r in Xt]
        mh, ma = models.fit_glm(Xk, yh), models.fit_glm(Xk, ya)
        ph = [models.predict_glm(mh, x) for x in Xtk]
        pa = [models.predict_glm(ma, x) for x in Xtk]
    ...
    for name, idx in groups.items():
        s = score(set(idx))
        rows_out.append({"group": name, "rmse_without": s, "rmse_full": full, "delta_rmse": s - full})
...
def learn_extra_innings_home_prior(rows: list[dict[str, Any]], prior_games: float = 80.0) -> dict[str, Any]:
    ...
    return {
        "active": n >= 200,
        "n": n,
        "home_probability": _clip(p, .45, .55),
        "prior_games": prior_games,
        "policy": "remains 0.50 until at least 200 authenticated examples",
    }
...
def learn_bookmaker_weights(rows: list[dict[str, Any]], min_games: int = 300) -> dict[str, Any]:
    ...
    usable = [
        r for r in rows
        if r.get("outcome") in (0, 1) and isinstance(r.get("book_probs"), dict) and r.get("book_probs")
    ]
    ...
    def predict(r: dict[str, Any], ww: dict[str, float]) -> float:
        vals = [
            (ww[b], _num((r.get("book_probs") or {}).get(b), .5))
            for b in books if (r.get("book_probs") or {}).get(b) is not None
        ]
        z = sum(x for x, _ in vals)
        return sum(x * p for x, p in vals) / z if z else .5

    def loss(ww: dict[str, float]) -> float:
        y = [int(r["outcome"]) for r in usable]
        p = [predict(r, ww) for r in usable]
...
def gap_bins(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for lo, hi, name in bands:
        xs = [
            ...
            and r.get("p_market") is not None
            and lo <= _num(r.get("p_model")) - _num(r.get("p_market")) < hi
        ]
        if not xs:
            continue
        y = [int(r["outcome"]) for r in xs]
        pm = [_clip(_num(r["p_model"], .5)) for r in xs]
        pk = [_clip(_num(r["p_market"], .5)) for r in xs]
        out[name] = {
            "n": len(xs),
            "mean_gap": sum(a - b for a, b in zip(pm, pk)) / len(xs),
            "model_brier": brier(y, pm),
            "market_brier": brier(y, pk),
...
def feature_drift(reference: list[list[float]], recent: list[list[float]]) -> dict[str, Any]:
    ...
    for j in range(1, p):
        ref = [r[j] for r in reference]
        rec = [r[j] for r in recent]
    ...
    return {
        "available": True,
        "features": out,
        "alerts": sorted(k for k, v in out.items() if v["alert"]),
    }
...
def reproducibility_manifest(
    ...
    contract = str((models.SCHEMA, models.FEATURE_NAMES))
    ...
    return {
        ...
        "code_sha": code_sha,
        "seed": int(seed),
        "split_policy": "strict temporal / walk-forward",
...
def full_report(rows: list[dict[str, Any]], labels: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema": SCHEMA,
        "games": len(paired),
        "baselines": baseline_predictions(paired),
        "walk_forward": walk_forward(rows, labels),
        "ablation": ablation_report(rows, labels),
