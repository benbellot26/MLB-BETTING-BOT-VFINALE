...
# Import V12.3 composition for its validated legacy-core compatibility setup.
# V13.9 then exposes a single explicit runner-facing engine boundary below.
from . import v123_entry as _v123_entry  # noqa: F401
...
from . import v138_book_telemetry, v138_native_evidence, v13_engine
...
v13_engine.install_runner_engine(runner)
...
def _tracked_allocate(results, *args, **kwargs):
    ...
    try:
        ...
        core.logging.exception(
            "V13.8.2 book telemetry capture failed; production probabilities remain unchanged"
        )
...
def _assert_v135_calibration_artifact():
    ...
    if (
        model.get("schema") != "v13-baseball-calibration-model-v2"
        or model.get("baseball_only") is not True
        or model.get("model_generation") != probability_contract.MODEL_GENERATION_FINGERPRINT
        or bad_status
    ):
        raise SystemExit(
            "V13.5.2 calibration artifact stale/incompatible: run `python -m v11.v13_train` before analysis"
        )
...
def _run_v135(*args, **kwargs):
    ...
    if isinstance(report, dict):
        report.setdefault("architecture", {})["runner_engine"] = runner.engine.status()
...
def self_test_v13():
    ...
    from . import (
        extra_innings_v13,
        v13_distribution_prior,
        v13_run_mean_runtime,
        v13_rich_run_shadow,
        uncertainty_v13,
    )

    assert config.VERSION.startswith("13.5")
    assert v13_engine.is_installed(runner)
    engine_status = runner.engine.status()
    assert engine_status["probability_neutral_research_boundary"] is True
    assert engine_status["version"].startswith("13.9-")
    ...
    assert contract.row_is_predictively_compatible(row)
    ...
    c = {
        "calibrators": {
            "GLOBAL": {"active": True, "method": "platt", "a": 0, "b": .5, "n": 999},
            "MARKET:ML": {"n": 40},
            "PHASE:EARLY:ML": {"n": 30},
            "PHASE:FINAL:ML": {"n": 7},
        }
    }
    assert cal.calibrate(.61, "ML", "FINAL", c)[0] == .61
    assert cal.calibrate(.61, "ML", "FINAL", c)[2] == 7
    u_hi = uncertainty_v13.empirical_interval(
        .55,
        calibration_n=30,
        phase_n=30,
        market_n=40,
    ...
    u_lo = uncertainty_v13.empirical_interval(
        .55,
        calibration_n=30,
        phase_n=30,
        market_n=40,
    ...
    if dist.get("active"):
        ...
        assert int(dist.get("exact_final_games") or 0) >= int(
            dist.get("exact_transfer_required_games") or 20
        )
        assert dist.get("exact_transfer_status") == "PASS_FINAL_ONLY"
        assert abs(d - float(dist.get("dispersion"))) < 1e-12
        assert abs(e - .08) < 1e-12 and meta.get("active")
    else:
    ...
    if mean_prior.get("active"):
        ...
        if mean_prior.get("model_generation") == contract.MODEL_GENERATION_FINGERPRINT:
            assert (
                observed < required
                or status != "PASS_FINAL_ONLY"
                or mean_prior.get("historical_candidate_active") is not True
            )
    h2, a2, m2 = v13_run_mean_runtime.apply_pair(5.0, 4.0, "LATE", mean_prior)
    assert h2 == 5.0 and a2 == 4.0 and not m2.get("active")

    rich = v13_rich_run_shadow.load()
    assert rich.get("active_for_production") is not True
    assert v13_daily_tracking._band(.004) == "0-1%"
    assert v13_daily_tracking._band(.12) == ">=10%"
    ...
    print(
        "SELF-TEST V13.9 ENGINE BOUNDARY + V13.8.2 EVIDENCE + GENERATION-FINGERPRINT + "
        "NATIVE CALIBRATION + ANALYTICS-ONLY GATES OK"
    )
