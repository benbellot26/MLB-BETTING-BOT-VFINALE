...
once, repeated snapshots cannot duplicate exposure, and portfolio limits persist
across every runtime call made on the same betting day.
"""
...
def _read(path:Path|str=LEDGER)->list[dict[str,Any]]:
    ...
    for line in target.read_text(encoding="utf-8").splitlines():
        ...
        if isinstance(row,dict) and row.get("schema") in {"pulsar-v14-official-bet-v1","pulsar-v14-official-bet-v2","pulsar-v14-official-bet-v3"}: rows.append(row)
    return rows
...
def _existing_market_key(row:dict[str,Any])->tuple[str,str]:
    market=str(row.get("canonical_market") or "") or _canonical_market({"market":row.get("market"),"selection":row.get("selection")})
...
def _existing_exposure(rows:list[dict[str,Any]],target_day:str)->tuple[float,dict[str,float],dict[str,float]]:
    ...
    for row in rows:
        ...
        stake=max(0.0,float(_num(row.get("stake_fraction")) or 0.0)); daily+=stake
        game=str(row.get("game_pk") or "UNKNOWN_GAME"); market=str(row.get("market") or "UNKNOWN")
        games[game]=games.get(game,0.0)+stake; markets[market]=markets.get(market,0.0)+stake
    return daily,games,markets


def _edge_rank(candidate:dict[str,Any])->tuple[float,float]:
    return float(_num(candidate.get("robust_edge_pp")) or -999.0),float(_num(candidate.get("model_edge_pp")) or -999.0)


def record_payload(payload:dict[str,Any],path:Path|str=LEDGER)->int:
    certification=payload.get("betting_certification") or {}
    if certification.get("certified") is not True: return 0
    rows=_read(path); occupied={_existing_market_key(row) for row in rows}; target_day=_target_day(payload)
    initial_daily,initial_games,initial_markets=_existing_exposure(rows,target_day)

    # Collapse competing sides/snapshots inside the current payload to the single
    # strongest candidate for each game x canonical market before allocating risk.
    ...
    for result_index,result in enumerate(results):
        ...
        for candidate in (result.get("decision") or {}).get("candidates") or []:
            ...
            enriched=dict(candidate); enriched["game_pk"]=game_pk; enriched["_result_index"]=result_index; enriched["_canonical_market"]=canonical
            previous=best.get(key)
            if previous is None or _edge_rank(enriched)>_edge_rank(previous): best[key]=enriched

    sized=size_portfolio(list(best.values()),certified=True,initial_daily_used=initial_daily,initial_game_used=initial_games,initial_market_used=initial_markets)
    added=0
