...
"""Immutable system-authorized bet ledger for Pulsar V14.

Rows are created only from current-generation, market-specifically certified BET
decisions. This ledger records what Pulsar authorized at the captured executable
price; it is NOT proof that a user actually placed the wager. Real executions
belong in the separate executed-bet ledger. A game/market can be authorized only
once, repeated snapshots cannot duplicate exposure, and portfolio limits persist
across every runtime call on the same betting day.
"""
...
def _read(path:Path|str=LEDGER)->list[dict[str,Any]]:
    ...
    for line in target.read_text(encoding="utf-8").splitlines():
        ...
        if isinstance(row,dict) and row.get("schema") in {"pulsar-v14-official-bet-v1","pulsar-v14-official-bet-v2","pulsar-v14-official-bet-v3","pulsar-v14-official-bet-v4","pulsar-v14-authorized-bet-v5"}: rows.append(row)
    return rows
...
def record_payload(payload:dict[str,Any],path:Path|str=LEDGER)->int:
    ...
    for result_index,result in enumerate(results):
        ...
        for candidate in (result.get("decision") or {}).get("candidates") or []:
            ...
            if int(candidate.get("sharp_sportsbook_source_count") or 0)<1: continue
...
def report(rows:list[dict[str,Any]])->dict[str,Any]:
    ...
    return {"schema":"pulsar-v14-authorized-bet-performance-v5","ledger_role":"SYSTEM_AUTHORIZED_BET","execution_confirmed":False,"bets":len(rows),"settled":len(settled_rows),"wins":sum(row.get("result")=="WIN" for row in settled_rows),"losses":sum(row.get("result")=="LOSS" for row in settled_rows),"pushes":sum(row.get("result")=="PUSH" for row in settled_rows),"profit_bankroll_fraction":profit,"staked_bankroll_fraction":staked,"roi":profit/staked if staked else None,"roi_scope":"system-authorized hypothetical execution at the captured price; not user-realized ROI","sharp_information_clv":_clv_summary(sharp),"execution_clv":_clv_summary(execution),"clv":{**_clv_summary(sharp),"definition":"compatibility alias for sharp-information CLV; use execution_clv for tradable price quality"},"by_market":by_market,"dedupe_policy":"one authorized bet per game x canonical market","staking_policy":"persistent slate-wide per-bet/game/market/day caps across runtime calls","write_gate":"current generation + global certification + market certification + qualified decision + sharp sportsbook source + executable price"}
...
def main()->None:
    ...
    if args.cmd=="record": print(f"PULSAR_V14_AUTHORIZED_BET_LEDGER added={record_payload(json.loads(Path(args.payload).read_text(encoding='utf-8')),args.ledger)}")
    else: settled_n=settle(args.ledger); out=write_report(args.ledger,args.report); print(f"PULSAR_V14_AUTHORIZED_BET_LEDGER settled={settled_n} bets={out['bets']}")
