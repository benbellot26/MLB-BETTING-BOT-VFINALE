...
def evaluate(*,prediction:dict[str,Any],market_snapshot:dict[str,Any],sharp_market:dict[str,Any],certification:dict[str,Any],starter_degraded:bool=False,execution_market:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    for key,market,price,book in _execution_rows(market_snapshot,execution_market):
        ...
        source_count=int(sharp_row.get("source_count") or 0)
        ...
        research_ready=edge_qualified and not blockers; market_certified=_certified_market(certification,canonical)
        ...
        rows.append({"selection":key,"canonical_market":canonical,"market":market,"execution_book":book,"execution_source":"LINE_SHOPPED" if (execution_market or {}).get("selections") else "CANONICAL_FALLBACK","price":price,"probability":p,"lower_probability":lower,"break_even_probability":breakeven,"model_edge_pp":model_edge,"robust_edge_pp":robust_edge,"sharp_edge_pp":sharp_edge,"robust_sharp_edge_pp":robust_sharp_edge,"sharp_source_count":source_count,"sharp_dispersion_pp":_num(sharp_row.get("dispersion_pp")),"model_edge_threshold_pp":model_threshold,"robust_edge_threshold_pp":robust_threshold,"robust_sharp_edge_threshold_pp":robust_sharp_threshold,**penalties,"edge_qualified":edge_qualified,"research_ready":research_ready,"market_betting_certified":market_certified,"status":status,"blockers":blockers})
    rows.sort(key=lambda r:r.get("robust_edge_pp") if r.get("robust_edge_pp") is not None else -999,reverse=True)
    return {"schema":"pulsar-v14-decision-diagnostics-v5","betting_certified":any(r.get("market_betting_certified") for r in rows),"starter_degraded":bool(starter_degraded),"market_freshness_verified":freshness,"recommendations_authorized":any(r.get("status")=="BET" for r in rows),"research_clv_collection_authorized":True,"threshold_policy":"market base + model uncertainty + sharp disagreement + lower-bound edge versus sharp","line_shopping_used":bool((execution_market or {}).get("selections")),"candidates":rows,"best":rows[0] if rows else None}
