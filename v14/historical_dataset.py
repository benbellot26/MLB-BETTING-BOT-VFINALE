...
def verify_manifest(manifest_path:Path=MANIFEST,base:Path=DATA_DIR)->dict[str,Any]:
    manifest=json.loads(manifest_path.read_text(encoding="utf-8")); expected={str(item.get("path")):str(item.get("sha256")) for item in manifest.get("files") or [] if str(item.get("path") or "").startswith("data/v137/team_")}; actual_paths=source_paths(base)
    ...
    for path in actual_paths:
        ...
        if digest!=expected[key]: raise ValueError(f"historical source hash mismatch: {key}")
        checked.append({"path":key,"sha256":digest})
    return {"schema":"pulsar-v14-historical-source-integrity-v1","verified":True,"dataset_content_sha256":manifest.get("dataset_content_sha256"),"feature_contract_sha256":manifest.get("feature_contract_sha256"),"source_files":checked,"source_file_count":len(checked)}
