...
def _compact_training_features(feature_row:dict[str,Any],prediction:dict[str,Any],statcast:dict[str,Any],research:dict[str,Any],pitch_matchup:dict[str,Any],*,analyzed_at:str,game_date:str)->dict[str,Any]:
    ...
    return {"schema":"pulsar-v14-training-features-v4","as_of":feature_row.get("as_of"),"point_in_time":feature_row.get("point_in_time") is True,"capture_mode":"PROSPECTIVE_LIVE_SNAPSHOT","pit_source_contract":pit,"base_run_projection":prediction.get("base_run_projection") or {},"context_adjustment":prediction.get("context_adjustment") or {},"home_starter":ctx.get("home_starter") or {},"away_starter":ctx.get("away_starter") or {},"home_lineup":{k:(ctx.get("home_lineup") or {}).get(k) for k in ("count","weighted_ops","coverage","status","players")},"away_lineup":{k:(ctx.get("away_lineup") or {}).get(k) for k in ("count","weighted_ops","coverage","status","players")},"bullpen":features.get("bullpen") or {},"operational":features.get("operational") or {},"environment":features.get("environment") or {},"statcast_shadow":statcast,"pitch_matchup_shadow":pitch_matchup,"research_challengers":research,"data_quality":quality}


def build_native_result(game:dict[str,Any],event:dict[str,Any],*,target_date:str,analyzed_at:str,input_builder:InputBuilder=build_game_inputs,starter_evidence_builder:StarterEvidenceBuilder=starter_integrity_evidence)->dict[str,Any]:
...
def build_candidate(target_date:str,*,analyzed_at:str|None=None,api_key:str|None=None,collector:Collector=collect_pregame_strict,input_builder:InputBuilder=build_game_inputs,starter_evidence_builder:StarterEvidenceBuilder=starter_integrity_evidence)->dict[str,Any]:
    ...
    for game in snapshot.games:
        ...
        try: results.append(build_native_result(game,event,target_date=target_date,analyzed_at=at,input_builder=input_builder,starter_evidence_builder=starter_evidence_builder))
        except Exception as exc: skipped.append({"game_pk":game_pk,"reason":f"{type(exc).__name__}: {exc}"})
    return {"schema":"pulsar-v14-native-candidate-v4","version":VERSION,"model_generation":MODEL_GENERATION,"role":"CANDIDATE_NON_PUBLISHING","native_acquisition":True,"legacy_acquisition_adapter":False,"market_probability_used_as_feature":False,"target_date":target_date,"analyzed_at":at,"results":results,"skipped":skipped,"coverage":{"scheduled_future_games":len(snapshot.games),"matched_odds_games":len(snapshot.matches),"priced_games":len(results),"skipped_games":len(skipped)}}
