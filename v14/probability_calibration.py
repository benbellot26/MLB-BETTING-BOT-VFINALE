...
def calibrate_probability(p:float,market:str,phase:str,artifact:dict[str,Any]|None=None)->tuple[float,dict[str,Any]]:
    ...
    if cal.get("active") is not True: return float(p),{"active":False,"accepted":_accepted(cal),"key":key,"method":"identity","n":int(cal.get("n") or 0),"status":str(cal.get("status") or "COLLECTING"),"holdout":cal.get("raw_holdout") or {}}
