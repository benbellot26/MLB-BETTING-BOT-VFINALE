...
"""Leakage-safe probability calibration research for Pulsar V14.

Calibrators may be fitted and validated in shadow, but a data artifact can never
silently change the published probability surface. Applying a non-identity
transform in production requires an explicit code-level probability-policy
promotion. The current policy therefore remains frozen even if a research
calibrator reaches ACTIVE_TRANSFORM.
"""
...
# Deliberately None for the current champion. A future calibration transform may
# be promoted only by an explicit code change together with a new
# PROBABILITY_POLICY_ID. A mutable JSON artifact alone can never activate it.
PRODUCTION_TRANSFORM_POLICY_ID: str|None = None
PRODUCTION_TRANSFORM_ACTIVATION_POLICY="EXPLICIT_NEW_PROBABILITY_POLICY_REQUIRED"
...
def _fit_one(items:list[tuple[float,int]],*,minimum_n:int)->dict[str,Any]:
    ...
    common={"candidate_slope":a,"candidate_intercept":b,"train_n":train_n,"holdout_n":holdout_n,"raw_holdout":raw,"calibrated_holdout":cal,"identity_diagnostics":identity,"brier_gain":brier_gain,"logloss_gain":logloss_gain,"paired_gain_ci95":gains,"stable_parameters":stable,"production_application_allowed":False}
    if active:return {**base,**common,"active":True,"accepted":True,"method":"platt-logit","status":"ACTIVE_TRANSFORM","slope":a,"intercept":b,"reason":"paired_oos_transform_gain_ci95_research_candidate"}
...
def build_artifact(rows:list[dict[str,Any]])->dict[str,Any]:
    eligible=_eligible_rows(rows);excluded_policy=sum(1 for r in rows if r.get("settled") and r.get("model_generation")==MODEL_GENERATION and r.get("probability_policy_id")!=PROBABILITY_POLICY_ID);market_rows=_latest_settled_by_game(eligible);phase_rows=_latest_settled_by_game_phase(eligible);calibrators={}
    ...
    return {"schema":"pulsar-v14-calibration-v3","model_generation":MODEL_GENERATION,"probability_policy_id":PROBABILITY_POLICY_ID,"generated_at":datetime.now(timezone.utc).isoformat(),"latest_observation_at":latest,"eligible_current_policy_rows":len(eligible),"excluded_other_policy_rows":excluded_policy,"strictly_pregame":True,"chronological_holdout":True,"chronological_order":"game_date_utc_then_analyzed_at","market_snapshot_policy":"latest strictly-pregame snapshot per game","phase_snapshot_policy":"latest strictly-pregame snapshot per (game, phase)","total_push_policy":"exclude pushes from binary calibration","market_probability_used_as_feature":False,"production_transform_policy_id":PRODUCTION_TRANSFORM_POLICY_ID,"production_transform_activation_policy":PRODUCTION_TRANSFORM_ACTIVATION_POLICY,"production_transform_application":"SHADOW_ONLY_UNTIL_EXPLICIT_POLICY_PROMOTION","calibrators":calibrators,"policy":{"market_min_n":MIN_MARKET_OBSERVATIONS,"phase_min_n":MIN_PHASE_OBSERVATIONS,"exact_probability_policy_required":True,"research_transform_nomination":"paired holdout Brier gain CI95 lower >0 + LogLoss lower >= -0.001 + stable parameters","identity_acceptance":"holdout ECE<=0.05, slope 0.80..1.20, |intercept|<=0.25","production_transform_activation":PRODUCTION_TRANSFORM_ACTIVATION_POLICY}}


def _empty_artifact()->dict[str,Any]:return {"schema":"pulsar-v14-calibration-v3","model_generation":MODEL_GENERATION,"probability_policy_id":PROBABILITY_POLICY_ID,"production_transform_policy_id":PRODUCTION_TRANSFORM_POLICY_ID,"production_transform_activation_policy":PRODUCTION_TRANSFORM_ACTIVATION_POLICY,"calibrators":{}}
...
def _production_transform_authorized(artifact:dict[str,Any],cal:dict[str,Any])->bool:
    return bool(cal.get("active") is True and PRODUCTION_TRANSFORM_POLICY_ID is not None and PRODUCTION_TRANSFORM_POLICY_ID==PROBABILITY_POLICY_ID and artifact.get("probability_policy_id")==PRODUCTION_TRANSFORM_POLICY_ID)
def calibrate_probability(p:float,market:str,phase:str,artifact:dict[str,Any]|None=None)->tuple[float,dict[str,Any]]:
    data=load_artifact() if artifact is None else _validated_artifact(artifact);key,cal=_select_calibrator(data,market,phase);research_active=cal.get("active") is True;authorized=_production_transform_authorized(data,cal);common={"probability_policy_id":PROBABILITY_POLICY_ID,"artifact_generated_at":data.get("generated_at"),"artifact_latest_observation_at":data.get("latest_observation_at"),"research_candidate_active":research_active,"production_transform_authorized":authorized,"production_activation_policy":PRODUCTION_TRANSFORM_ACTIVATION_POLICY}
    if not authorized:
        return float(p),{**common,"active":False,"accepted":_accepted(cal),"production_transform_applied":False,"key":key,"method":"identity","research_method":str(cal.get("method") or "identity"),"n":int(cal.get("n") or 0),"status":str(cal.get("status") or "COLLECTING"),"holdout":cal.get("raw_holdout") or {},"candidate_slope":cal.get("candidate_slope") if cal.get("candidate_slope") is not None else cal.get("slope"),"candidate_intercept":cal.get("candidate_intercept") if cal.get("candidate_intercept") is not None else cal.get("intercept")}
    a=_num(cal.get("slope"));b=_num(cal.get("intercept"))
    if a is None or b is None:return float(p),{**common,"active":False,"accepted":False,"production_transform_applied":False,"key":key,"method":"identity","n":int(cal.get("n") or 0),"status":"INVALID_ARTIFACT"}
    return _sigmoid(a*_logit(_clip(p))+b),{**common,"active":True,"accepted":True,"production_transform_applied":True,"key":key,"method":"platt-logit","n":int(cal.get("n") or 0),"slope":a,"intercept":b,"status":str(cal.get("status") or "ACTIVE_TRANSFORM"),"holdout":cal.get("calibrated_holdout") or {}}
def calibrate_surface(probabilities:dict[str,Any],*,phase:str,artifact:dict[str,Any]|None=None)->tuple[dict[str,float],dict[str,Any]]:
    ...
    return surface,{"schema":"pulsar-v14-calibration-application-v3","model_generation":MODEL_GENERATION,"probability_policy_id":PROBABILITY_POLICY_ID,"artifact_schema":data.get("schema"),"artifact_generated_at":data.get("generated_at"),"artifact_latest_observation_at":data.get("latest_observation_at"),"phase":str(phase).upper(),"markets":details,"any_active":any(v.get("active") for v in details.values()),"any_research_candidate_active":any(v.get("research_candidate_active") for v in details.values()),"production_transform_applied":any(v.get("production_transform_applied") for v in details.values()),"all_accepted":bool(details) and all(v.get("accepted") for v in details.values()),"production_activation_policy":PRODUCTION_TRANSFORM_ACTIVATION_POLICY}
...
def main()->None:
    parser=argparse.ArgumentParser(description="Fit leakage-safe Pulsar V14 probability calibrators");parser.add_argument("command",choices=["fit"]);parser.add_argument("--predictions",default=str(PREDICTIONS));parser.add_argument("--output",default=str(ARTIFACT));args=parser.parse_args();artifact=write_artifact(args.predictions,args.output);active=sum(bool(v.get("active")) for v in artifact.get("calibrators",{}).values());accepted=sum(bool(v.get("accepted")) for v in artifact.get("calibrators",{}).values());print(f"PULSAR_V14_CALIBRATION research_active={active} accepted={accepted} production_transform_policy={PRODUCTION_TRANSFORM_POLICY_ID} output={args.output}")
