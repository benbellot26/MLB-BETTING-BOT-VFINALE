...
def validate_candidate_coverage(candidate:dict[str,Any])->None:
    coverage=candidate.get("coverage") or {}; matched=int(coverage.get("matched_odds_games") or 0); priced=int(coverage.get("priced_games") or 0)
...
def validate_production_payload(payload:dict[str,Any])->None:
    ...
    if top_cert and top_cert.get("model_generation") not in {None,MODEL_GENERATION}: raise ValueError("payload betting certification generation mismatch")
    for result in payload.get("results") or []:
        ...
        if certification and certification.get("model_generation") not in {None,MODEL_GENERATION}: raise ValueError(f"game {game_pk} certification generation mismatch")
