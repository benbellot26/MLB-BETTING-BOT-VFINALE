...
def validate_candidate_coverage(candidate: dict[str, Any]) -> None:
    ...
    if matched > 0:
        ...
        if ratio < MIN_PRICED_MATCHED_COVERAGE:
            raise RuntimeError(f"native V14 priced/matched coverage too low: {priced}/{matched}={ratio:.1%} < {MIN_PRICED_MATCHED_COVERAGE:.0%}")
...
def validate_production_payload(payload: dict[str, Any]) -> None:
    ...
    for result in payload.get("results") or []:
        if result.get("model_generation") != MODEL_GENERATION: raise ValueError(f"game {result.get('game_pk')} is not V14")
        if result.get("native_acquisition") is not True: raise ValueError(f"game {result.get('game_pk')} is not native acquisition")
        prediction = result.get("v14_prediction") or {}
        if prediction.get("model_generation") != MODEL_GENERATION or prediction.get("role") != "PRODUCTION": raise ValueError(f"game {result.get('game_pk')} missing V14 production prediction")
        if prediction.get("market_probability_used_as_feature") is not False: raise ValueError(f"game {result.get('game_pk')} used market probability as model feature")
        surface = prediction.get("probabilities") or {}
        pairs = ((surface.get("away_ml"), surface.get("home_ml")), (surface.get("away_plus_1_5"), surface.get("home_minus_1_5")), (surface.get("home_plus_1_5"), surface.get("away_minus_1_5")), (surface.get("over"), surface.get("under")))
        for left, right in pairs:
            if left is None or right is None or abs(float(left) + float(right) - 1.0) > 1e-9: raise ValueError(f"game {result.get('game_pk')} has invalid probability surface")
        if "market_snapshot" not in result or "market_diagnostics" not in result: raise ValueError(f"game {result.get('game_pk')} missing market audit state")

        certification = result.get("betting_certification") or top_cert or {}
        decision = result.get("decision") or {}
        for candidate in decision.get("candidates") or []:
            if candidate.get("status") == "BET" and certification.get("certified") is not True:
                raise ValueError(f"game {result.get('game_pk')} emitted BET without statistical certification")
        fallback = result.get("starter_fallback") or {}
        if fallback.get("degraded") and any(c.get("status") == "BET" for c in decision.get("candidates") or []):
            raise ValueError(f"game {result.get('game_pk')} emitted BET with degraded starter data")
...
def build_persisted(*, target_date: str | None = None, destination: Path | str = V14_PAYLOAD, candidate_destination: Path | str = V14_CANDIDATE) -> dict[str, Any]:
    ...
    payload["authorization_basis"] = {
        ...
        "betting_certified": bool((payload.get("betting_certification") or {}).get("certified")),
        "note": "Publication is analytics/software authorization; betting requires the independent statistical certification gate.",
    }
