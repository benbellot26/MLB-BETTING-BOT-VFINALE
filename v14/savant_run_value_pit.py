...
def baserunning_prior_snapshot(target_season:int,*,getter:UserTextGetter=_get_text)->dict[str,Any]:
    prior=int(target_season)-1; params={"csv":"true","game_type":"Regular","n":1,"season_end":prior,"season_start":prior,"split":"no","team":"","type":"Run","with_team_only":1}; url=_url(BASERUN_URL,params); rows=_rows(getter(url)); return {"schema":"pulsar-v14-savant-baserunning-prior-v1","role":"SHADOW_DATA_ONLY","provider":"Baseball Savant Baserunning Run Value CSV","source_url":url,"target_season":int(target_season),"source_season":prior,"point_in_time":True,"current_target_season_results_used":False,"raw_columns":list(rows[0].keys()),"rows":rows}
