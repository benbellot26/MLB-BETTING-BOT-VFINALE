...
are filtered by actual game timestamp before the target game. Provider failure
is fail-soft because this evidence is challenger-only: the model falls back to
season workload rather than failing a production slate or inventing observations.
"""
...
def recent_starts(player_id:Any,season:int,game_date:Any,*,getter:Getter=http_json,limit:int=8)->list[dict[str,Any]]:
    ...
    try:
        ...
        return []
    try: splits=((payload.get("stats") or [{}])[0].get("splits") or [])
    except Exception: return []
    rows=[]
    for split in splits:
        ...
        try: batters_faced=float(stat.get("battersFaced")) if stat.get("battersFaced") is not None else None
        except Exception: batters_faced=None
        rows.append({"game_date":str(raw_date),"innings":ip,"pitches":pitches,"batters_faced":batters_faced,"source":"MLB gameLog filtered strictly before target"})
...
def enrich_starter(starter:dict[str,Any],game_date:Any,*,getter:Getter=http_json)->dict[str,Any]:
    row=dict(starter or {}); pid=row.get("id")
    try: season=int(str(game_date)[:4])
    except Exception: season=0
    starts=recent_starts(pid,season,game_date,getter=getter) if pid and season else []
    row["recent_starts"]=starts
    row["recent_starts_pit_safe"]=True
    row["recent_starts_n"]=len(starts)
    row["recent_starts_status"]="READY_SHADOW" if starts else "COLLECTING"
    row["recent_starts_cutoff"]="strictly before target game timestamp/calendar day"
    return row
