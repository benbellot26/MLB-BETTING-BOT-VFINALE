...
def refresh(cutoff_day:str,*,lookback_days:int=45,priors_path:Path|str=DEFAULT_PRIORS,report_path:Path|str=DEFAULT_REPORT)->dict[str,Any]:
    cutoff=date.fromisoformat(str(cutoff_day)[:10]); days=max(7,min(120,int(lookback_days))); start=cutoff-timedelta(days=days)
    artifact=build(cutoff.isoformat(),season_start=start.isoformat()); priors=dict(artifact.get("priors") or {}); coverage=artifact.get("coverage") or {}
    if priors.get("point_in_time") is not True or priors.get("stable_id_only") is not True: raise ValueError("enriched Statcast priors lost PIT/stable-ID contract")
    if int(coverage.get("hitter_pitch_split_players") or 0)<=0: raise ValueError("enriched Statcast artifact contains no hitter pitch-type splits")
