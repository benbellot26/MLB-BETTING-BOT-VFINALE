...
uses a bounded Baseball Savant detailed-CSV request, dedupes pitch rows, then
applies the V14 enrichment layer that adds shrinkable hitter pitch-type splits.
It never calls current-season summary endpoints for an old game. For a cutoff
D, only pitch rows with game_date < D may contribute. Output is research-only
and cannot auto-promote a champion.
"""
...
from v11.v137_free_data import STATCAST_ROW_CAP, dedupe_statcast_rows
...
def _query_params(start_day:str,end_day:str,season:int)->dict[str,Any]:
    """Use Savant's complete detailed-CSV request shape.

    Savant's UI/client contract includes explicit empty filters and
    ``group_by=name`` even for pitch-level ``type=details`` downloads. Sparse
    requests can still return rows while silently omitting usable pitch-type
    values. Keeping the complete request shape makes the pitch vocabulary an
    explicit source contract instead of assuming provider defaults.
    """
    return {
        "all":"true",
        "hfPT":"",
...
def _fetch_statcast_rows_full(
    ...
    *,
    ...
    text=fetch_text(
        ...
        _query_params(start_day,end_day,season),
        timeout=45,
    )
    # Strip a possible UTF-8 BOM before DictReader and normalize every key as a
    # second defensive layer because provider/header behavior is not stable.
    rows=[_normalize_row(row) for row in csv.DictReader(io.StringIO((text or "").lstrip("\ufeff")))]
    if rows:
        columns=set(rows[0])
        required={"game_date","batter","pitcher","pitch_type"}
        if not required.issubset(columns):
            raise ValueError(f"unexpected Statcast detailed CSV schema; missing={sorted(required-columns)}")
    return rows


def _fetch_statcast_rows_adaptive_v14(
    start_day:str,
    end_day:str,
    *,
    season:int,
    ...
    """Adaptive detailed-CSV fetch; never trust a response at the row cap."""
    ...
    diagnostics:dict[str,Any]={"requests":[],"cap_hits":0,"splits":0,"unresolved_truncation":False,"query_contract":"SAVANT_COMPLETE_DETAILS_V1"}

    def _fetch(a:date,b:date)->list[dict[str,str]]:
        rows=_fetch_statcast_rows_full(a.isoformat(),b.isoformat(),season=season,fetch_text=fetch_text)
        capped=len(rows)>=int(row_cap)
        diagnostics["requests"].append({"start":a.isoformat(),"end":b.isoformat(),"rows":len(rows),"cap_hit":capped})
        ...
        span=(b-a).days+1;left_days=max(1,span//2);left_end=a+timedelta(days=left_days-1);right_start=left_end+timedelta(days=1)
        diagnostics["splits"]+=1
        return _fetch(a,left_end)+_fetch(right_start,b)

    rows=dedupe_statcast_rows(_fetch(start,end));diagnostics["deduped_rows"]=len(rows);diagnostics["requests_made"]=len(diagnostics["requests"])
