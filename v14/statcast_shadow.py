...
hitter pitch-type splits and falls back to the audited V137 artifact for basic
shadow features when the enriched refresh is unavailable.
"""
...
def load_priors(path:Path|str|None=None)->dict[str,Any]:
    ...
    enriched=_read(STATCAST_PRIORS)
    return enriched if enriched else _read(LEGACY_STATCAST_PRIORS)
...
def _artifact_state(payload:dict[str,Any],target_date:str)->tuple[bool,str,int|None]:
    ...
    return True,("FRESH" if age<=FRESH_DAYS else "AGING" if age<=AGING_DAYS else "STALE"),age


def _shrink(value:Any,pa:int,prior:float,k:float)->float:
    raw=_num(value); weight=max(0.0,float(pa))/(max(0.0,float(pa))+k)
    return prior if raw is None else weight*raw+(1-weight)*prior
...
def _lineup_feature(lineup:dict[str,Any],hitters:dict[str,Any],freshness:str)->dict[str,Any]:
    ...
    for i,player in enumerate((lineup.get("players") or [])[:9]):
        ...
        reliability=pa/(pa+HITTER_SHRINK_PA); w=order_weights[min(i,8)]*max(.10,reliability); covered.append({"id":pid,"pa":pa,"reliability":reliability,"pitch_type_split_count":len(prior.get("pitch_type_splits") or {})})
...
def build_shadow_features(feature_row:dict[str,Any],*,target_date:str,artifact:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    base={"schema":"pulsar-v14-statcast-shadow-v3","role":"SHADOW_ONLY","auto_activation":False,"point_in_time":safe,"target_date":str(target_date),"artifact_schema":priors.get("schema") if priors else None,"artifact_cutoff_day":priors.get("cutoff_day") if priors else None,"artifact_age_days":age,"freshness":freshness,"promotion_ready":safe and freshness!="STALE","pitch_matchup_data_available":bool(priors.get("v14_enrichment",{}).get("hitter_pitch_type_splits")) if priors else False,"reason":"safe_prior_cutoff" if safe else "artifact_unavailable_or_unsafe"}
