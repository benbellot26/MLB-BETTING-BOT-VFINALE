...
def _lineup_feature(lineup:dict[str,Any],hitters:dict[str,Any],freshness:str)->dict[str,Any]:
    ...
    for i,player in enumerate((lineup.get("players") or [])[:9]):
        ...
        reliability=pa/(pa+HITTER_SHRINK_PA); w=order_weights[min(i,8)]*max(.10,reliability); covered.append({"id":pid,"pa":pa,"reliability":reliability})
...
def build_shadow_features(feature_row:dict[str,Any],*,target_date:str,artifact:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    base={"schema":"pulsar-v14-statcast-shadow-v2","role":"SHADOW_ONLY","auto_activation":False,"point_in_time":safe,"target_date":str(target_date),"artifact_cutoff_day":priors.get("cutoff_day") if priors else None,"artifact_age_days":age,"freshness":freshness,"promotion_ready":safe and freshness!="STALE","reason":"safe_prior_cutoff" if safe else "artifact_unavailable_or_unsafe"}
