...
def snapshot_rows(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for result in payload.get("results") or []:
        ...
        rows.append({
            "schema": "pulsar-v14-prediction-record-v3",
            ...
            "probabilities": {key: _num(probabilities.get(key)) for key in (
...
def append_snapshot(payload: dict[str, Any], path: Path | str = PREDICTIONS) -> int:
    ...
    for row in snapshot_rows(payload):
        index[(str(row.get("game_pk") or ""), str(row.get("analyzed_at") or ""))] = row
...
def performance_report(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    markets: dict[str,list[tuple[float,int]]]=defaultdict(list); run_errors=[]; total_errors=[]
    for row in settled:
        hs=int(row["home_score"]); aws=int(row["away_score"]); probs=row.get("probabilities") or {}; line=_num(row.get("total_line"))
        vals=(("ML","home_ml",int(hs>aws)),("RL_HOME_-1.5","home_minus_1_5",int(hs-aws>=2)),("RL_AWAY_-1.5","away_minus_1_5",int(aws-hs>=2)))
        for name,key,y in vals:
    ...
    all_items=[item for values in markets.values() for item in values]; overall=_binary_metrics(all_items)
    overall["interpretation"]="dashboard-only; correlated markets must not drive model promotion"
    return {
        "schema":"pulsar-v14-performance-v3","model_generation":MODEL_GENERATION,"generated_at":datetime.now(timezone.utc).isoformat(),
        ...
        "overall":overall,"calibration":_calibration(all_items),"markets":{n:{**_binary_metrics(v),"calibration":_calibration(v)} for n,v in sorted(markets.items())},
        ...
        "clv":{"status":"UNAVAILABLE","reason":"No official bet ledger plus canonical closing-price feed; market movement proxy is reported separately."},
