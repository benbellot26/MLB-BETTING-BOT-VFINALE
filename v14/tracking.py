...
def snapshot_rows(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for result in payload.get("results") or []:
        ...
        probabilities=prediction.get("probabilities") or {}; projection=prediction.get("run_projection") or {}
        total_line=_num(projection.get("total_line"))
        if total_line is None: total_line=_num((result.get("canonical_lines") or {}).get("TOTAL"))
        rows.append({"schema":"pulsar-v14-prediction-record-v3","model_generation":MODEL_GENERATION,"game_pk":str(result.get("game_pk") or ""),"target_date":target_date or str(game_date or "")[:10],"game_date":game_date,"analyzed_at":analyzed_at,"phase":result.get("phase") or prediction.get("phase"),"home":result.get("home"),"away":result.get("away"),"home_mu":_num(projection.get("home_mu")),"away_mu":_num(projection.get("away_mu")),"total_line":total_line,"probabilities":{k:_num(probabilities.get(k)) for k in ("home_ml","away_ml","home_minus_1_5","away_plus_1_5","away_minus_1_5","home_plus_1_5","over","under")},"market_snapshot":result.get("market_snapshot") or {},"market_diagnostics":result.get("market_diagnostics") or {},"settled":False,"home_score":None,"away_score":None,"settled_at":None})
    return rows
...
def append_snapshot(payload: dict[str, Any], path: Path | str=PREDICTIONS) -> int:
    ...
    for row in snapshot_rows(payload): index[(str(row.get("game_pk") or ""),str(row.get("analyzed_at") or ""))]=row
    _write_jsonl(path,sorted(index.values(),key=lambda r:(str(r.get("target_date") or ""),str(r.get("game_pk") or ""),str(r.get("analyzed_at") or ""))))
    return len(index)-before


def _final_scores(game: dict[str, Any]) -> tuple[int,int] | None:
    status=((game.get("status") or {}).get("abstractGameState") or "").lower(); detailed=((game.get("status") or {}).get("detailedState") or "").lower()
    ...
    return None if home is None or away is None else (int(home),int(away))
...
def _binary_metrics(items: list[tuple[float,int]]) -> dict[str,Any]:
    if not items: return {"n":0,"brier":None,"log_loss":None,"accuracy_50":None,"mean_probability":None,"observed_rate":None}
    eps=1e-12
...
def _canonical_settled(rows:list[dict[str,Any]])->tuple[list[dict[str,Any]],int]:
    ...
    return sorted(latest.values(),key=lambda r:(str(r.get("target_date") or ""),str(r.get("game_pk") or ""))),len(records)


def _price(row:dict[str,Any],market:str,selection:str)->float|None:
    markets=((row.get("market_snapshot") or {}).get("markets") or {}); return _num((((markets.get(market) or {}).get("selections") or {}).get(selection) or {}).get("price"))
...
def performance_report(rows:list[dict[str,Any]])->dict[str,Any]:
    settled,count=_canonical_settled(rows); markets=defaultdict(list); run_errors=[]; total_errors=[]
    for row in settled:
        hs=int(row["home_score"]); aws=int(row["away_score"]); probs=row.get("probabilities") or {}; line=_num(row.get("total_line"))
        for name,key,y in (("ML","home_ml",int(hs>aws)),("RL_HOME_-1.5","home_minus_1_5",int(hs-aws>=2)),("RL_AWAY_-1.5","away_minus_1_5",int(aws-hs>=2))):
    ...
    return {"schema":"pulsar-v14-performance-v3","model_generation":MODEL_GENERATION,"generated_at":datetime.now(timezone.utc).isoformat(),"prediction_records_settled":count,"games_settled":len(settled),"canonical_snapshot_policy":"latest strictly-pregame snapshot per game","overall":overall,"calibration":_calibration(all_items),"markets":{n:{**_binary_metrics(v),"calibration":_calibration(v)} for n,v in sorted(markets.items())},"runs":{"team_run_mae":sum(run_errors)/len(run_errors) if run_errors else None,"total_run_mae":sum(total_errors)/len(total_errors) if total_errors else None},"roi":{"status":"UNAVAILABLE","reason":"No official bet/stake ledger in V14 analytics-only production."},"clv":{"status":"UNAVAILABLE","reason":"No official bet ledger plus canonical closing-price feed; market movement proxy is reported separately."},"market_movement_proxy":_market_movement_proxy(rows)}


def write_performance(path:Path|str=PREDICTIONS,report_path:Path|str=PERFORMANCE)->dict[str,Any]:
    rows=_read_jsonl(path); report=performance_report(rows); report["selection_feedback"]=load_pick_performance(); report["controlled_learning"]=learning_report(rows,MODEL_GENERATION)
...
def main()->None:
    ...
    settle=sub.add_parser("settle"); settle.add_argument("--predictions",default=str(PREDICTIONS)); settle.add_argument("--report",default=str(PERFORMANCE)); args=parser.parse_args()
    if args.command=="snapshot":
        payload=json.loads(Path(args.payload).read_text(encoding="utf-8")); print(f"PULSAR_V14_TRACKING snapshot_added={append_snapshot(payload,args.predictions)}")
    else:
        settled=settle_predictions(args.predictions); report=write_performance(args.predictions,args.report); print(f"PULSAR_V14_TRACKING settled={settled} games={report['games_settled']} learning={report['controlled_learning']['stage']}")
