...
Professional scoring rules in this module are deliberately game-paired.  A
Pulsar-vs-sharp gain is never computed from different samples.  The report also
keeps raw-vs-calibrated metrics, phase slices and rolling windows so calibration
and concept drift can be detected before betting authorization.
"""
...
def snapshot_rows(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for result in payload.get("results") or []:
        ...
        probabilities=prediction.get("probabilities") or {}; raw=prediction.get("raw_probabilities") or probabilities; projection=prediction.get("run_projection") or {}
        total_line=_num(projection.get("total_line")); total_line=total_line if total_line is not None else _num((result.get("canonical_lines") or {}).get("TOTAL"))
        keys=("home_ml","away_ml","home_minus_1_5","away_plus_1_5","away_minus_1_5","home_plus_1_5","over","under")
        rows.append({
            "schema":"pulsar-v14-prediction-record-v5","model_generation":MODEL_GENERATION,
            "game_pk":str(result.get("game_pk") or ""),"target_date":target_date or str(game_date or "")[:10],"game_date":game_date,"analyzed_at":analyzed_at,
            "phase":result.get("phase") or prediction.get("phase"),"home":result.get("home"),"away":result.get("away"),
            "home_mu":_num(projection.get("home_mu")),"away_mu":_num(projection.get("away_mu")),"total_line":total_line,
            "probabilities":{k:_num(probabilities.get(k)) for k in keys},"raw_probabilities":{k:_num(raw.get(k)) for k in keys},
            "calibration":prediction.get("calibration") or {},"probability_intervals":prediction.get("probability_intervals") or {},
            "market_snapshot":result.get("market_snapshot") or {},"market_diagnostics":result.get("market_diagnostics") or {},"sharp_market":result.get("sharp_market") or {},
            "decision":result.get("decision") or {},"training_features":result.get("training_features") or {},"starter_fallback":result.get("starter_fallback") or {},
...
def _final_scores(game: dict[str, Any]) -> tuple[int,int] | None:
    ...
    teams=game.get("teams") or {}; home=_num((teams.get("home") or {}).get("score")); away=_num((teams.get("away") or {}).get("score"))
...
def _binary_metrics(items: list[tuple[float,int]]) -> dict[str,Any]:
    ...
    return {"n":len(items),"brier":sum((p-y)**2 for p,y in items)/len(items),
            "log_loss":-sum(y*math.log(max(eps,min(1-eps,p)))+(1-y)*math.log(max(eps,min(1-eps,1-p))) for p,y in items)/len(items),
            "accuracy_50":sum((p>=.5)==bool(y) for p,y in items)/len(items),"mean_probability":sum(p for p,_ in items)/len(items),"observed_rate":sum(y for _,y in items)/len(items),
...
def _mean_ci(values: list[float], z: float=1.96) -> dict[str,Any]:
    ...
    var=sum((x-mean)**2 for x in values)/(len(values)-1); se=math.sqrt(var/len(values))
    return {"n":len(values),"mean":mean,"ci95_lower":mean-z*se,"ci95_upper":mean+z*se,"std_error":se}


def _paired_benchmark(items: list[tuple[float,float,int]]) -> dict[str,Any]:
    """Compare model and sharp on exactly the same games."""
    if not items:
        return {"n":0,"paired_n":0,"brier":None,"log_loss":None,"model_paired":_binary_metrics([]),"brier_gain_vs_sharp":None,"logloss_gain_vs_sharp":None,"brier_gain_ci95_lower":None,"logloss_gain_ci95_lower":None}
    eps=1e-12; model=[(m,y) for m,_s,y in items]; sharp=[(s,y) for _m,s,y in items]
    brier_diff=[]; ll_diff=[]
    for m,s,y in items:
        # Positive means Pulsar is better than sharp.
        brier_diff.append((s-y)**2-(m-y)**2)
        mll=-(y*math.log(max(eps,min(1-eps,m)))+(1-y)*math.log(max(eps,min(1-eps,1-m))))
        sll=-(y*math.log(max(eps,min(1-eps,s)))+(1-y)*math.log(max(eps,min(1-eps,1-s))))
        ll_diff.append(sll-mll)
    sm=_binary_metrics(sharp); mm=_binary_metrics(model); bg=_mean_ci(brier_diff); lg=_mean_ci(ll_diff)
    return {**sm,"n":len(items),"paired_n":len(items),"model_paired":mm,
            "brier_gain_vs_sharp":bg["mean"],"logloss_gain_vs_sharp":lg["mean"],
            "brier_gain_ci95_lower":bg["ci95_lower"],"brier_gain_ci95_upper":bg["ci95_upper"],
            "logloss_gain_ci95_lower":lg["ci95_lower"],"logloss_gain_ci95_upper":lg["ci95_upper"],
            "paired_inference":"paired per-game score differences; normal 95% CI"}
...
def _market_observations(rows: list[dict[str,Any]]) -> dict[str,dict[str,list[Any]]]:
    out={m:{"model":[],"raw":[],"paired":[]} for m in ("ML","RL_HOME_-1.5","RL_AWAY_-1.5","TOTAL_OVER")}
    for row in rows:
        hs=int(row["home_score"]); aws=int(row["away_score"]); probs=row.get("probabilities") or {}; raw=row.get("raw_probabilities") or probs; line=_num(row.get("total_line"))
        definitions=[("ML","home_ml",int(hs>aws)),("RL_HOME_-1.5","home_minus_1_5",int(hs-aws>=2)),("RL_AWAY_-1.5","away_minus_1_5",int(aws-hs>=2))]
        if line is not None: definitions.append(("TOTAL_OVER","over",int(hs+aws>line)))
...
def _market_report(rows: list[dict[str,Any]]) -> dict[str,Any]:
    ...
    for name,data in obs.items():
        metrics=_binary_metrics(data["model"]); metrics["calibration"]=_calibration(data["model"]); metrics["raw_metrics"]=_binary_metrics(data["raw"]); metrics["sharp_benchmark"]=_paired_benchmark(data["paired"]); report[name]=metrics
    return report
...
def _quality_segments(rows: list[dict[str,Any]]) -> dict[str,Any]:
    degraded=[r for r in rows if bool((r.get("starter_fallback") or {}).get("degraded") or (r.get("starter_fallback") or {}).get("degraded_sides"))]
    clean=[r for r in rows if r not in degraded]
    return {"STARTER_CLEAN":{"games":len(clean),"markets":_market_report(clean)},"STARTER_DEGRADED":{"games":len(degraded),"markets":_market_report(degraded)}}
...
def _rolling_segments(rows: list[dict[str,Any]]) -> dict[str,Any]:
    ...
    for days in (30,60,90):
        cutoff=anchor-timedelta(days=days); scoped=[r for r in rows if _chronological_key(r)[0]>=cutoff]
        out[f"{days}d"]={"games":len(scoped),"through":anchor.isoformat(),"markets":_market_report(scoped)}
    return out


def performance_report(rows: list[dict[str,Any]]) -> dict[str,Any]:
    settled,record_count=_canonical_settled(rows); markets=_market_report(settled); all_items=[]; run_errors=[]; total_errors=[]
    for values in _market_observations(settled).values(): all_items.extend(values["model"])
    for row in settled:
    ...
    return {"schema":"pulsar-v14-performance-v5","model_generation":MODEL_GENERATION,"generated_at":datetime.now(timezone.utc).isoformat(),
            "prediction_records_settled":record_count,"games_settled":len(settled),"canonical_snapshot_policy":"latest strictly-pregame snapshot per game, exact game_date ordering",
            "overall":overall,"calibration":_calibration(all_items),"markets":markets,
            "segments":{"phase":_phase_segments(settled),"data_quality":_quality_segments(settled),"rolling":_rolling_segments(settled)},
            ...
            "roi":{"status":"UNAVAILABLE","reason":"No certified official bet/stake sample yet."},
            "clv":{"status":"UNAVAILABLE","n":0,"mean_clv":None,"reason":"Use official/paper ledgers and verified close feed; market movement proxy is separate."},
...
def main() -> None:
    ...
    rep=sub.add_parser("report"); rep.add_argument("--predictions",default=str(PREDICTIONS)); rep.add_argument("--report",default=str(PERFORMANCE))
