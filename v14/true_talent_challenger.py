...
def offense_component(lineup_statcast:dict[str,Any],pitch_matchup:dict[str,Any],baserunning:dict[str,Any]|None=None)->dict[str,Any]:
    xwoba=_num(lineup_statcast.get("xwoba")); hard=_num(lineup_statcast.get("hard_hit_rate")); barrel=_num(lineup_statcast.get("barrel_rate")); k_minus_bb=_num(lineup_statcast.get("k_minus_bb_rate")); matchup=_num(pitch_matchup.get("matchup_xwoba")); running=_num((baserunning or {}).get("baserunning_run_adjustment"))
    values=[]
    ...
    # For hitters, K%-BB% is a cost: a larger value is worse offense.
    if k_minus_bb is not None: values.append(((.12-k_minus_bb)/.10,.10,"inverse_k_minus_bb"))
    if not values: return {"role":ROLE,"status":"COLLECTING","promotion_ready":False,"auto_activation":False,"reason":"offense evidence missing"}
    ...
    source_ready=(lineup_statcast.get("promotion_ready") is True and pitch_matchup.get("status")=="READY_SHADOW" and (baserunning or {}).get("status")=="READY_SHADOW")
    return {"role":ROLE,"status":"READY_SHADOW","promotion_ready":source_ready,"auto_activation":False,"score":score,"run_adjustment":run_adjustment,"components":[{"name":n,"z":v,"weight":w} for v,w,n in values],"baserunning_run_adjustment":running,"source_readiness":{"lineup_statcast":lineup_statcast.get("promotion_ready") is True,"pitch_matchup":pitch_matchup.get("status")=="READY_SHADOW","baserunning":(baserunning or {}).get("status")=="READY_SHADOW"}}
...
def prevention_component(starter:dict[str,Any],bullpen:dict[str,Any],defense:dict[str,Any],expected_starter_ip:float|None)->dict[str,Any]:
    ...
    if missing: return {"role":ROLE,"status":"COLLECTING","promotion_ready":False,"auto_activation":False,"missing":missing}
    ...
    source_ready=(starter.get("promotion_ready") is True and bullpen.get("promotion_ready") is True and defense.get("status")=="READY_SHADOW")
    return {"role":ROLE,"status":"READY_SHADOW","promotion_ready":source_ready,"auto_activation":False,"expected_starter_ip":sip,"pitching_factor":pitching,"prevention_factor":combined,"run_environment_direction":"factor>1 means easier opponent scoring","source_readiness":{"starter_statcast":starter.get("promotion_ready") is True,"bullpen_statcast":bullpen.get("promotion_ready") is True,"defense_catcher":defense.get("status")=="READY_SHADOW"}}
...
def build(*,statcast:dict[str,Any],pitch_matchup:dict[str,Any],research:dict[str,Any])->dict[str,Any]:
    ...
    components=(home_off,away_off,home_prev,away_prev); ready=all(x.get("status")=="READY_SHADOW" for x in components); promotion_ready=ready and all(x.get("promotion_ready") is True for x in components)
    return {"schema":"pulsar-v14-true-talent-challenger-v2","role":ROLE,"auto_activation":False,"status":"READY_SHADOW" if ready else "COLLECTING","promotion_ready":promotion_ready,"home_offense":home_off,"away_offense":away_off,"home_prevention":home_prev,"away_prevention":away_prev,"ablation_required":True,"market_probability_used_as_feature":False}
