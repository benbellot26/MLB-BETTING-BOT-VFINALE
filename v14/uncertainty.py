...
When enough settled observations exist, market×phase×probability-bucket error
bands are loaded from `v14_uncertainty.json`.  Otherwise the previous wide
heuristic floors remain.  These are decision-safety bands, not Bayesian credible
intervals.
"""
...
def _load(path:Path|str=ARTIFACT)->dict[str,Any]:
    ...
    return row if isinstance(row,dict) and row.get("schema")=="pulsar-v14-uncertainty-fit-v1" else {}
...
def _empirical_half(market:str,phase:str,p:float,artifact:dict[str,Any])->tuple[float|None,dict[str,Any]]:
    ...
    for scope in (str(phase or "ALL").upper(),"ALL"):
        ...
        if cell.get("ready") is True and _num(cell.get("empirical_half_width")) is not None:
            return float(cell["empirical_half_width"]),{"cell":f"{market}:{scope}:{bucket}","n":int(cell.get("n") or 0),"source":"empirical"}
...
def intervals(probabilities:dict[str,Any],calibration:dict[str,Any]|None,*,data_quality:dict[str,Any]|None=None,starter_degraded:bool=False,market_fresh:bool|None=None,artifact:dict[str,Any]|None=None)->dict[str,Any]:
    cal=calibration or {}; cal_markets=cal.get("markets") or {}; phase=str(cal.get("phase") or "ALL").upper(); empirical=_load() if artifact is None else artifact; penalty=_quality_penalty(data_quality,starter_degraded=starter_degraded,market_fresh=market_fresh); selections={}
    for market,(left,right) in PAIR_MAP.items():
        p=_num(probabilities.get(left))
        if p is None: continue
        meta=cal_markets.get(market) or {}; n=int(meta.get("n") or 0); active=meta.get("active") is True; accepted=meta.get("accepted") is True or active
        ...
        if empirical_half is not None: half=max(.020,empirical_half)
            ...
            holdout=meta.get("holdout") or {}; ece=abs(_num(holdout.get("ece")) or 0.0); sampling=1.64*math.sqrt(max(1e-8,p*(1-p))/max(25,n)); half=max(.025,sampling+.5*ece)
        else: half=BASE_HALF_WIDTH[market]
        half=min(.18,half+penalty); lo,hi=max(0.0,p-half),min(1.0,p+half)
        selections[left]={"probability":p,"lower":lo,"upper":hi,"half_width_pp":100*half,"calibration_active":active,"calibration_accepted":accepted,"evidence_n":max(n,int(evidence["n"])),"uncertainty_source":evidence["source"],"uncertainty_cell":evidence["cell"]}
        rp=1-p; selections[right]={**selections[left],"probability":rp,"lower":1-hi,"upper":1-lo}
    return {"schema":"pulsar-v14-probability-uncertainty-v2","method":"empirical-calibration-aware-decision-band-with-conservative-fallback","phase":phase,"starter_degraded":bool(starter_degraded),"market_freshness_verified":market_fresh,"quality_penalty_pp":100*penalty,"selections":selections,"note":"Decision-safety intervals; not exact Bayesian credible intervals."}
